...
│   ChaCha20Poly1305Impl   HkdfSha256   SealedApiKeyCipher       │
...
The shaded components below close the gameplay-readiness gaps a project hits
once a room has more than one player in it:

| Component | Purpose |
|-----------|---------|
| `INetworkObjectPool` | Pluggable object pool for high-churn spawning |
| `NetworkManager.TransportFactoryFn` | Pluggable transport (mock-transport integration tests; a transport that reaches the gateway another way) |
...
| `NetworkManager.Send()`             | Main thread only    | Reads negotiated session state unsynchronised; the defensive buffer copy does not make the call thread-safe |
| `NetworkVariable.Value` (write)     | Main thread only    | Never from background threads |
| `NetworkVariable.Value` (read)      | Main thread only    | Plain field read — no fence. `NetworkVariableVector3` (12 B) and `NetworkVariableQuaternion` (16 B) exceed any architecture's atomic width, and no struct assignment is atomic in C# whatever its size, so an off-thread read can observe a half-written value |
...
| WebGL | — | **Not supported** — the factory replaces the socket, not the background thread above it, and the browser player has no threads. See [Troubleshooting](troubleshooting.md#webgl-is-not-a-supported-platform) |
...
the `LocalEndPoint`, which is what the SDK reports and logs as this client's
address. No address is bound into the handshake: `HandshakeInit` seals the API
key with an empty AAD, so authentication does not depend on the probe's answer.

If the probe fails (isolated containers, no default route) the SDK logs a warning
and falls back to loopback. The handshake still authenticates; what a loopback
`LocalEndPoint` usually means is that datagrams are not leaving the host at all,
and the warning makes that diagnosable rather than a silent timeout.
...
`ReliableChannel` holds the client-side ARQ state for that path.

⚠️ The bit is a **request**, not a guarantee. The AEAD pipeline clears it from
any frame it is not emitting an ARQ sub-header for — the flags byte is covered
by the associated data, so advertising a sub-header the frame does not carry
would make the gateway read a truncated sub-header region and drop the packet.
A downgrade is therefore silent on the wire by design, and audible only through
the two advisories named under `RoomManager` below.
...
Every packet after the handshake is encrypted and authenticated. The stack is
X25519 for key agreement, Ed25519 for server identity, HKDF-SHA256 for key
derivation and ChaCha20-Poly1305 for the per-packet seal; the server's static
key is pinned, and `NetworkSettings.serverPinningMode` decides what happens when the
key it presents is not the pinned one.

Nothing above `AeadPipeline` sees ciphertext, a nonce or a key. Application
code hands the SDK a payload and receives one; the handshake, the key schedule
and the per-packet seal are performed by `HandshakeHandler` and `AeadPipeline`
and are not part of the API this document describes. The two behaviours that do
reach an integrator — what happens when the nonce space runs out, and what
happens to a replayed packet — are below.
...
All application-level packets carry a fixed binary header followed by a
variable-length payload. The header is written and read by `PacketBuilder` and
`PacketParser`; nothing above the transport layer needs to know its layout.
...
**Event wiring helper.** `InitialiseNetwork()`, `Connect()`, and
...
Handles room CRUD with `FLAG_RELIABLE`. Whether that bit survives to the wire
is decided by the session, not by the call: acknowledged delivery needs
`NetworkSettings.EmitArqSequence` (on by default) **and** the gateway to have
negotiated `CapabilityFlags.ArqAck`. Without both, the packet ships once,
best-effort, and `ReliableSendAdvisory` or `PeerCapabilityAdvisory` names which
of the two was missing. Exposes C# events
for all room lifecycle outcomes. Internally maintains a
`PendingCreateTable<CreateRoomOptions>` — an id-keyed table capped at 16 pending
creates (`MaxPendingCreates`) and swept on a TTL, so an unanswered create is
abandoned rather than held for ever.
...
- `Despawn(objectId)` — destroys locally (or releases to the pool) and
  broadcasts to all peers.  Only the object's owner may: the gateway relays a
  despawn from nobody else and answers the refusal with no packet, so anyone
  else's call is refused here rather than removing the object from this client
  alone.  `CanDespawn` / `TryDespawn` ask the same question without the console
  error.
- `MarkAllVariablesDirtyForResync()` — re-flags every `NetworkVariable`
...
- `PruneDestroyed()` sweeps the dictionary in one pass and evicts every
...
**Writing your own variable type.** `NetworkVariable<T>` is public and abstract:
derive from it and implement `Serialize(BinaryWriter)` and
`Deserialize(BinaryReader)`. `Deserialize` must apply the value it reads through
**`ApplyFromWire`**, which stores it, raises `OnValueChanged` on this replica,
and leaves the variable clean so a replica never re-publishes what it was told:

```csharp
public sealed class NetworkVariableByte : NetworkVariable<byte>
{
    // Required: the base constructor takes the owner and the member name, and
    // neither has a default — a derived type that declares no constructor of
...
⚠️ **Do not use `SetValueWithoutNotify` in `Deserialize`.** Its contract is that
nothing is raised, so a variable applied that way is silent on every client but
the owner — the defect the shipped types were repaired for in v8.0.0. `Serialize`
returns `void` and the entry length is already decided when it is called, so a
value that cannot be written must be refused *before* it is stored, not while it
is being serialised.
...
```
  GameManager.OnConnected()
       ...
       │  Spawner.SetObjectPool(pool)          ← optional
       │
...
**Solution:** On every `RoomManager.OnPlayerJoined` event, every
...
`ip_migration_key`. The token is preserved wherever the SDK
still believes it can resume. The ordinary case is the heartbeat-liveness
timeout (`OnHeartbeatTimeout`), which now fires
...
Two narrower cases keep it as well, and both are attempts to resume that never
got far enough to spend it: a `Reconnect()` that timed out without a validated
`Challenge`, and a pin store that could not be read during a reconnect — a fault
the SDK itself classes as transient.

Every other exit path — an explicit `Disconnect()`, a first connect that timed
out, a transport `SocketException`, a server-initiated `Disconnect`, or a
...
| First connect times out (`ConnectionTimeoutRoutine`)             | **Cleared** — there was no token yet | `Timeout` |
| `Reconnect()` times out with no validated `Challenge`            | **Preserved** — the attempt never spent it | `Timeout` |
| Pin configuration refuses during a reconnect (`SendReconnectInit`)    | **Preserved** — classed as transient | `ProtocolError` |
...
`ConnectionLost` and `Timeout` can each appear with **either** preserved or
cleared snapshots, so the reason alone never settles it — distinguish at runtime
by checking `NetworkManager.CanReconnect` inside your `OnDisconnected` handler.
...
2. TransformPacketBuilder.BuildUpdatePayloadInto(buffer, offset, objectId, transformState, inputTick)
     ...
     → 0 bytes when the pose carries a non-finite position or scale.  The
       gateway refuses such a frame on arrival, so it was costing a datagram
       and an update with nothing said on the client that produced it.  A
       coordinate has no nearest valid substitute the way a non-unit rotation
       does, so the record is refused rather than repaired — the caller skips
       the send AND leaves its last-sent baseline where it was, or the next
       threshold comparison is made against NaN and the field never broadcasts
       again.
...
4. NetworkManager.EncryptAndSend(rawPacket)
     ...
     AeadPipeline.Seal(header, payload)
       binds the header to the payload, draws a fresh nonce from the session's
       counter, and marks the header as sealed
     → encrypted byte[]
...
3. PacketGates.ValidateHeader(data, length, out packetType, out wasEncrypted)
     → HeaderValidationResult, one of
       Ok | TooShort | BadMagic | UnsupportedVersion | MalformedFlags | UnknownType
     The verdict is the return value and the only two facts it hands back are
     the two out-parameters; anything else the header carries is re-read from
     the bytes downstream.  Both the gate and the enum are `internal` — game
     code does not call this seam, it observes the outcome.

4. NetworkManager.DecryptInboundPacket(data, length)
     Is FLAG_ENCRYPTED set?  yes →
     AeadPipeline.Open(header, ciphertext)
       verifies the header it was bound to, refuses a replayed or pre-window
       sequence, and returns the payload with its original sequence restored
...
7. NetworkObjectRegistry.Get(objectId) → nb
   NetworkTransformInterpolator.AddState(state, timestamp)   // TransformState carries pos/rot/scale
...
*RTMPE SDK 1.0.5 — [Getting Started](getting-started.md) — [API Reference](api/index.md)*
