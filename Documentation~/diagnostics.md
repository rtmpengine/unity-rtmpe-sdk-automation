...
> **These hosts need the RTMPE repository.** The `make` verbs above and the
> Conversion Wizard all drive the same headless conversion engine, and that
> engine is a .NET project inside the RTMPE repository — it is not part of this
> UPM package. A project that installed the package alone therefore has no host
> to run: the `make` verbs are not present, and the wizard reports that it found
> none and offers a field to point it at a checkout.
>
> Without a checkout, this page is the substitute for the reason string: the
> table below states, per rule, every condition under which a fix is withheld.
> Compare your declaration against the row for the rule you expected a fix from,
> and apply the edit by hand.
...
conversion engine is reachable**. The wizard launches that engine from an RTMPE
repository checkout: inside the repository it resolves on its own, and elsewhere
it asks for the folder once and remembers it per user. With no checkout on the
machine there is no engine to launch, and conversion is a manual edit guided by
...
constant. A type that repeats an id among its own constructions is reported from
that type alone, in any analysis scope; a type that reuses an id its base class
already holds is reported at the end of compilation, because only then is the
base's contribution in hand. Two constructions in opposite
...
[Preconditions](#preconditions). Nothing in a Unity project supplies that
ledger, so in Unity convert through a host — which owns the ledger — and where
no host is reachable, apply the conversion by hand against the shape recorded
under [Preconditions](#preconditions).
