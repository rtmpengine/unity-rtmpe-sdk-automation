...
> SDK Version: `com.rtmpe.sdk 2.0.21`
> Protocol Version: **v4** — Magic `0x5254` ("RT")
...
──────  ────  ───────────  ──────────────────────────────────────────────────
  ...
  2       1   version      4  (protocol version 4)
...
| Bit 6  | `0x40` | `SealedApiKey`    | `HandshakeInit` (0x05) carries the API key as an X25519 sealed box (see §4 Step 1). **Mandatory** — an init without it is refused. **Client → server only**, set solely on `HandshakeInit` |
...
| `0x45` | `PhysicsSync`         | C→S       | no       | Custom binary — rigidbody state; see §10 |
...
────  ────────────────  ─────  ────  ──────────────────────────────────────────────
  1   HandshakeInit     0x05   C→S   [eph_pub:32][ChaCha20-Poly1305(api_key)]  (FLAG_SEALED_API_KEY set)
...
unauthenticated peer. There is exactly one envelope format, declared by the
`SealedApiKey` flag (0x40, §2); the gateway refuses a `HandshakeInit` that
leaves the flag clear, before it reads the payload.

**Sealed-box envelope.** The API key is sealed to the gateway's static X25519
public key as an anonymous sealed box. No shared secret is distributed: the
developer needs only the gateway's public key, the same value surfaced as the
dashboard Sealed-Box Public Key. Nothing secret therefore ships inside a game
binary, which is what a symmetric envelope could never avoid.

```
plaintext    = [api_key_len:2 LE u16][api_key_bytes:N][client_ephemeral_pub:32]
...
unique for every seal. The box is at least 82 bytes (32-byte ephemeral key +
2-byte length prefix + 32-byte key commitment + 16-byte Poly1305 tag). The
gateway opens it with its matching X25519 private key.

**`client_ephemeral_pub` is the key this connection will offer in Round 2**, and
it is what makes Round 2's key the *authenticated* client's key rather than
whoever's packet arrives second. The Round-1 echo (§ below) binds the exchange to
a Round-1 packet, but that packet is plaintext on the wire and an on-path relay
observes it too, so echoing it proves only that the sender watched the same
conversation. The commitment rides under the same AEAD tag as the API key: a
relay may copy the envelope but cannot restate its contents, so it cannot have
the gateway complete ECDH with a key of its own and be issued a session under the
authenticating project. The gateway refuses a Round 2 offering any other key, and
refuses a Round 1 that carries no commitment — an optional binding is declined by
omission. Generate the X25519 pair before building the envelope.

**AAD is empty.** An earlier revision bound the ciphertext to
...
|------------------|-------------------|
| `HandshakeInit`  | `[eph_pub:32][ct+tag:N]`, ≥ 82 bytes, `FLAG_SEALED_API_KEY` set — see §4 Step 1 |
...
### Client → server: rigidbody state (`PhysicsSync` — 0x45)

`NetworkRigidbody` and `NetworkRigidbody2D` do **not** send under `StateSync`.
Their payload is variable-length and its length range overlaps the transform
shapes above, so a receiver reading both under one opcode would have to guess
which grammar it holds. They carry their own type instead
(`PhysicsPacketBuilder`, gateway `packet::physics`), unreliable like `StateSync`:

```
[object_id:8 LE u64]                                            // offset 0
[changed_mask:1 u8]                                             // offset 8
[if changed_mask & 0x01 (position)]:
    3-D: [x:4][y:4][z:4] LE f32          2-D: [x:4][y:4] LE f32
[if changed_mask & 0x02 (rotation)]:
    3-D: [x:4][y:4][z:4][w:4] LE f32     2-D: [angle:4 LE f32]
[if changed_mask & 0x04 (velocity)]:
    3-D: [x:4][y:4][z:4] LE f32          2-D: [x:4][y:4] LE f32
[if changed_mask & 0x08 (angular velocity)]:
    3-D: [x:4][y:4][z:4] LE f32          2-D: [z:4 LE f32]
[if changed_mask & 0x10 (sleep)]:        [is_sleeping:1 u8]  0 or 1
[if changed_mask & 0x20 (constraints)]:  [constraint_mask:1 u8]
```

The high two bits of `changed_mask` are the **dimensionality marker**, not data:
`0x40` marks a 3-D body and `0x80` a 2-D one, and the marker is what selects the
field widths above. A builder sets exactly one. A payload with neither is
rejected; a payload with both is read as 2-D — the 2-D arm does not consult the
3-D bit while the 3-D arm requires the 2-D bit clear, and the gateway's
well-formedness check takes the same reading so both sides compute the same
expected length. The low six bits (`DataFieldMask = 0x3F`) are the present-field
bits; any bit outside the mask and its marker is rejected rather than skipped.

Minimum size: **9 bytes** (object_id + mask, no fields). Maximum size:
9 + 12 + 16 + 12 + 12 + 1 + 1 = **63 bytes** for 3-D, and
9 + 8 + 4 + 8 + 4 + 1 + 1 = **35 bytes** for 2-D. Both sides compute the
expected length from the mask and require an exact match, so a truncated or
padded payload is dropped rather than parsed short.
...
*RTMPE SDK 2.0.21 — [Getting Started](getting-started.md) — [Architecture](architecture.md) — [API Reference](api/index.md)*
