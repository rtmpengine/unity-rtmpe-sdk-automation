...
| `TickAlignedSampling`| `bool`  | `false` | Broadcast the pose held on the tick boundary rather than at the frame that sends it — see [Tick-aligned sampling](#tick-aligned-sampling) |
...
| `AdaptiveDelay`        | `bool`   | `false` | Float the delay between the two-tick floor and `InterpolationDelay` according to the measured arrival spread, instead of holding the fixed value |
| `OwnerTickTimeline`    | `bool`   | `false` | Build the render timeline from the owning client's input tick rather than the server's broadcast tick — see [Owner-tick timeline](#owner-tick-timeline) |
| `MaxExtrapolationSeconds` | `float` | `0.15` | How long to continue along the last measured velocity when the render cursor outruns the newest snapshot. `0` restores freeze-then-resume |
...
Three settings decide how smooth a remote object looks. All three are **off by
default**, so a project that does not opt in behaves exactly as before.

### Owner-tick timeline

A broadcast record carries two clocks: the server's broadcast tick and the
owning client's input tick. By default the interpolator builds its render
timeline from the **server** tick. That tick is a uniform emission counter, but
...
*RTMPE SDK 2.0.21 — [Getting Started](../getting-started.md) — [Architecture](../architecture.md) — [Protocol Reference](../protocol.md)*
