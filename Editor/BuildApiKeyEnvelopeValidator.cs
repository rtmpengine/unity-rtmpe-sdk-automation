...
//
// Build-time guard: a Player build must ship the gateway's sealed-box public
// key in a NetworkSettings asset, because that is the only API-key envelope the
// gateway accepts.  Without it, every connection fails at the handshake with a
// Timeout that is only diagnosable after the binary is distributed.  Failing the build here turns
                ...
                if (guids != null)
                {
                    foreach (string guid in guids)
                    {
                        ...
                        if (ApiKeyEnvelopeCheck.IsConfigured(
                                settings.apiKeySealServerPublicKeyHex))
                        {
                            configuredCount++;
                        }
                        else
            ...
            switch (ApiKeyEnvelopeCheck.ClassifyBuildState(assetCount, configuredCount, emptyCount))
            {
                case EnvelopeBuildVerdict.NoAssets:
                    ...
                    Debug.LogWarning(
                        ...
                        "(the gateway's X25519 key), or the build will fail to connect.");
                    return;
                ...
                case EnvelopeBuildVerdict.NoneConfigured:
                    throw new BuildFailedException(
                        ...
                        "envelope.  A build cannot connect without one — the gateway accepts no other " +
                        "way to carry the API key.  Set apiKeySealServerPublicKeyHex (the gateway's " +
                        "X25519 key) on the NetworkSettings asset used by your NetworkManager — via " +
                        "Project Settings > RTMPE or the Setup Wizard — then rebuild.  " +
