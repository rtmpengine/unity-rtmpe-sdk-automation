...
namespace RTMPE.Editor
{
    ...
    internal static class WizardConfigValidator
    {
        ...
        /// key is required for the same reason: it is the only envelope the gateway
        /// accepts, so an asset without it produces a build that cannot connect.
        /// Returns (false, reason) naming the specific missing piece, or
        /// (true, null) when the configuration is connectable.
        /// </summary>
            ...
            string apiKey, int port, string pinnedKey, string sealKey)
        {
            ...
            if (!IsHex64(sealKey))
                return (false, "Paste the 64-hex API-key seal public key (the gateway's X25519 key, " +
                               "Connection settings in the dashboard). It is the only API-key envelope " +
                               "the gateway accepts, so a build without it cannot connect.");
            return (true, null);
        }
