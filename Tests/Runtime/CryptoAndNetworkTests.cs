...
namespace RTMPE.Tests
{
    ...
    public class NetworkVariableStringWireFormatTests
    {
        ...
        {
            var v = new NetworkVariableString(_owner, "v1", "");
        ...
        public void Serialize_AsciiString_WritesTwoByteLengthThenAsciiBytes()
        {
            ...
            var v = new NetworkVariableString(_owner, "v2", value);
        ...
        public void Serialize_LengthPrefixIsLittleEndian_NotVarint()
        {
            ...
            var v = new NetworkVariableString(_owner, "v3", value);
        ...
        public void RoundTrip_AsciiString_IsPreserved()
        {
            ...
            var src = new NetworkVariableString(_owner, "v4", original);
            // A distinct name: both live on one behaviour, and one behaviour
            // refuses two variables that derive one identity. Nothing here reads
            // the id — Serialize/Deserialize are called directly.
            var dst = new NetworkVariableString(_owner, "v4-readback", "");
        ...
        public void RoundTrip_UnicodeString_IsPreserved()
        {
            ...
            var src = new NetworkVariableString(_owner, "v5", original);
            var dst = new NetworkVariableString(_owner, "v5-readback", "");
    ...
    public class FlushDirtyVariablesDelegateTests
    {
        ...
        public void FlushDirtyVariables_WithDirtyVar_InvokesSendCallbackWithNonEmptyPayload()
        {
            ...
            var v = new NetworkVariableInt(_nbA, "v1", 0);
    ...
    public class H2_ValueLenFramingTests
    {
        ...
        public void SerializeWithId_WritesVariableId_ThenLength_ThenBytes()
        {
            ...
            var v = new NetworkVariableInt(_owner, "v7", 0x12345678);
            ...
            var bytes = ms.ToArray();
            // [var_id:4 LE][value_len:2 LE][value_bytes:4]
            Assert.AreEqual(10, bytes.Length, "int32 entry must be 10 bytes total.");

            uint varId = (uint)(bytes[0] | (bytes[1] << 8) | (bytes[2] << 16) | (bytes[3] << 24));
            Assert.AreEqual(v.VariableId, varId, "var_id must be the id the variable derived.");

            ushort valueLen = (ushort)(bytes[4] | (bytes[5] << 8));
        ...
        public void SerializeWithId_UnknownId_DoesNotCorruptSubsequentVariable()
        {
            ...
            //   [unknown_id][value_len=4][garbage:4]
            //   [known_id][value_len=4][value=42]
            using var ms = new MemoryStream();
            using var bw = new BinaryWriter(ms);

            const uint unknownId = 9999u;

            // Unknown entry
            bw.Write(unknownId);      // var_id
            bw.Write((ushort)4);      // value_len
            bw.Write(new byte[4]);    // garbage payload

            // Known entry — the id is derived from the type and the member name.
            var v = new NetworkVariableInt(_owner, "v1", 42);

            // The premise of the case, stated rather than assumed: the first
            // entry is only a skip if its id is one no variable here derives.
            Assert.AreNotEqual(unknownId, v.VariableId,
                               "the fixture's 'unknown' id must not be one the known variable owns");
            v.SerializeWithId(bw);
            bw.Flush();
            ...
            Assert.IsTrue(anyApplied, "The known variable must be reached after skipping the unknown entry.");
        ...
        {
            var v1 = new NetworkVariableInt(_owner,   "v10", 100);
            var v2 = new NetworkVariableFloat(_owner, "v11", 3.14f);
            var v3 = new NetworkVariableInt(_owner,   "v12", -999);
            ...
            // 🔑 On a SECOND behaviour of the same type. The readers must derive
            // the same ids as the writers — which they do, because the identity
            // folds in the concrete type and the member name and nothing else —
            // and putting them on the same behaviour would be two variables
            // claiming one identity, which registration refuses.
            var readerGo = new GameObject("Reader_H2");
            try
            {
                var reader = readerGo.AddComponent<NetworkBehaviourStub>();
                var r1 = new NetworkVariableInt(reader,   "v10", 0);
                var r2 = new NetworkVariableFloat(reader, "v11", 0f);
                var r3 = new NetworkVariableInt(reader,   "v12", 0);
                ...
                while (ms.Position < ms.Length)
                {
                    ...
                    if      (id == r1.VariableId) r1.Deserialize(br);
                    else if (id == r2.VariableId) r2.Deserialize(br);
                    else if (id == r3.VariableId) r3.Deserialize(br);
    ...
    public class LateJoinResyncTests
    {
        ...
        {
            var v = new NetworkVariableInt(_owner, "v1", 0);
            v.Value = 7;          // dirty
            v.MarkClean();         // cleaned
        ...
        public void SpawnManager_Resync_OnlyMarksOwnedObjects()
        {
            ...
            var vOwned   = new NetworkVariableInt(owned,   "v10", 0);
            var vForeign = new NetworkVariableInt(foreign, "v20", 0);
