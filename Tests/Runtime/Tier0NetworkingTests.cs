...
namespace RTMPE.Tests
{
    ...
    {
        // The driver runs TWO of these loops, on one interval and two clocks:
        // a simulation half charged with Time.deltaTime, which advances the CSP
        // tick and dispatches OnFixedTick, and a replication half charged with
        // Time.unscaledDeltaTime, which flushes dirty NetworkVariables so a
        // paused game still replicates.  Both loops have the shape Drain models,
        // so the helper below describes ONE of them.
        //
        // Rather than hijack Unity's clock, this fixture exercises the state
        // machine directly.  When the production code changes, this helper must
        // change in lock-step or the test stops protecting the invariant — and
        // it stopped once already, when the second loop was added and this
        // fixture kept describing a driver with one.
        ...
        [Test]
        [Description("MaxTicksPerFrame bounds EACH loop, so one frame's worst case is twice " +
                     "the ceiling — eight simulation ticks and eight variable flushes.")]
        public void BothLoops_30sPause_CostTwiceTheCeiling()
        {
            // Stated because the constant's own comment reads as though it
            // bounded the frame.  It bounds a loop; there are two.
            float simAcc = 0f, replicationAcc = 0f;
            int perFrame = Drain(ref simAcc, 30f) + Drain(ref replicationAcc, 30f);

            Assert.AreEqual(2 * MaxTicksPerFrame, perFrame);
        }

        [Test]
        [Description("The two accumulators have independent phase, so a frame that runs a " +
                     "simulation tick does not necessarily run a flush.")]
        public void TheTwoLoopsDoNotShareAPhase()
        {
            // A variable written inside OnFixedTick is published within one
            // tick interval, not necessarily in the same frame: after a pause
            // of an odd number of frames the two halves separate permanently,
            // and at a frame rate that is an integer multiple of the tick rate
            // they never coincide again.
            float simAcc = 0f, replicationAcc = 0f;
            const float Frame = 1f / 60f;

            Drain(ref simAcc, 0.5f * TickInterval);   // one half starts out of step

            bool sawSimWithoutFlush = false;
            for (int i = 0; i < 240; i++)
            {
                bool sim   = Drain(ref simAcc, Frame) > 0;
                bool flush = Drain(ref replicationAcc, Frame) > 0;
                if (sim && !flush) sawSimWithoutFlush = true;
            }

            Assert.IsTrue(sawSimWithoutFlush,
                "the phases are independent; a rule that assumes otherwise is wrong");
        }
    ...
    public class NetworkVariableTimestampTests
    {
        ...
        {
            var v = new NetworkVariableInt(_nb, memberName: "v0", initialValue: 0);
