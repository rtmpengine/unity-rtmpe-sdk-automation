...
namespace RTMPE.Tests
{
    ...
    public class NetworkManagerTests
    {
        ...
        // ── C-008: a legacy HandshakeAck must change nothing at all ───────────
        //
        // 🚨 These two cases previously asserted the OPPOSITE of the property
        // below, and in doing so pinned a defect (`CORE-RD-04`): the handler
        // tore the connection down on a bare 13-byte plaintext header, so a
        // single spoofed datagram ended every handshake this client attempted.
        //
        // The original reasoning was sound about what must not happen —
        // accepting a `HandshakeAck` would leave the session keys null while the
        // state machine reported Connected, and every later packet would go out
        // in plaintext. It does not follow that the packet should DISCONNECT.
        // Ignoring it cannot promote state either, and it leaves an in-progress
        // handshake to finish.
        //
        // ⛔ Nothing emits a `HandshakeAck`: the gateway builds one only in a
        // unit test, and a genuinely legacy server would stamp protocol version
        // 3 and be refused at the header gate. Its arrival is evidence about the
        // sender, not about the session.

        [Test]
        [Description("A legacy HandshakeAck arriving mid-handshake must leave the handshake " +
                     "in progress — it is unauthenticated and proves nothing.")]
        public void LegacyHandshakeAck_WhileConnecting_LeavesTheHandshakeAlone()
        {
            ...
            _manager.OnDisconnected += _ => disconnected = true;

            _manager.SimulateLegacyHandshakeAckForTests();

            Assert.AreEqual(NetworkState.Connecting, _manager.State,
                "An unauthenticated HandshakeAck must not move the state machine. One spoofed " +
                "13-byte header would otherwise end every connection attempt this client makes.");
            Assert.IsFalse(disconnected,
                "OnDisconnected must not be raised by a packet that carries no key material.");
            Assert.IsFalse(_manager.IsConnected,
                "Nor may it promote the session: ignoring is not accepting.");
        }

        [Test]
        [Description("A legacy HandshakeAck arriving while Disconnected must be a no-op.")]
