...
namespace RTMPE.Tests.Runtime
{
    ...
    public class PhysicsSyncSecurityTests
    {
        ...
        public void ApplyRemoteState_RateLimit_BurstDroppedAfterCapacity()
        {
            ...
            // Admission is counted by the state actually moving, because the
            // component records no packet counter: the merge is the only
            // observable difference between an admitted packet and a dropped
            // one.  Positions start at 1 so the first admitted packet differs
            // from the zero the component starts at — a first packet of (0,0,0)
            // is admitted and changes nothing, and would be counted as a drop.
            //
            // The step stays far below maxPositionDeltaPerTick so the delta cap
            // never fires; the token bucket is the only gate under test, and it
            // sits ahead of that cap in any case.
            int accepted = 0;
            for (int i = 1; i <= 50; i++)
            {
                Vector3 before = GetField<PhysicsState>(_nrb, "_receivedState").Position;
                _nrb.ApplyRemoteState(
                    new PhysicsState { Position = new Vector3(i * 0.001f, 0f, 0f) },
                    PhysicsPacketBuilder.ChangedPosition);
                Vector3 after = GetField<PhysicsState>(_nrb, "_receivedState").Position;
