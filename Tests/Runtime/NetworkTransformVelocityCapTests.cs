...
// send after spawn skips the check because
...
//                                               dating the candidate to the
//                                               current frame; the send path
//                                               dates it to the tick boundary
//                                               when tick-aligned sampling is on
...
//   • OwnerTeleportTo(newPos)                 — carries the baseline to
//                                               the destination; the cap stays
//                                               armed for travel after it.
//
...
namespace RTMPE.Tests
{
    ...
    public class NetworkTransformVelocityCapTests
    {
        ...
        {
            // Adversarial: a baseline dated ahead of the candidate yields a
            // non-positive dt, and no velocity can be derived from it.  The
            // helper must short-circuit (dt <= 0)
        ...
        // ── OwnerTeleportTo carries the baseline ──────────────────────────────

        [Test]
        [Description("OwnerTeleportTo moves the velocity-cap baseline to the destination rather than suspending the cap.")]
        public void OwnerTeleportTo_CarriesBaseline_JumpNotChargedAsTravel()
        {
            ...
            // Measured from the destination the owner has not moved, so the
            // kilometre it jumped is not charged.  What the cap does to travel
            // AFTER the teleport needs a clock this runner cannot advance —
            // Time.unscaledTimeAsDouble only moves between frames — and is
            // covered headlessly by OwnerVelocityCapGuardTests.
            var result = _nt.ClampOwnerVelocityForTest(teleportTarget);
            Assert.AreEqual(teleportTarget, result);
        }
