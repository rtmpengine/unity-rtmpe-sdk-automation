...
namespace RTMPE.Tests
{
    ...
    public class PacketBuilderTests
    {
        ...
        [Test]
        public void Build_HeaderVersion_IsTheProtocolConstant()
        {
            // Read from the constant rather than spelled: the gateway refuses any
            // other value, so a literal here would pin this suite to whichever
            // version was current when it was written.
            Assert.AreEqual(PacketProtocol.VERSION, _builder.BuildHeartbeat()[2]);
            Assert.AreEqual(PacketProtocol.VERSION, _builder.BuildDisconnect()[2]);
            Assert.AreEqual(PacketProtocol.VERSION, _builder.BuildHandshakeInit(new byte[1], sealedApiKey: true)[2]);
        }
        ...
        {
            var pkt = _builder.BuildHandshakeInit(new byte[8], sealedApiKey: true);
        ...
        public void BuildHandshakeInit_Length_Is13PlusPayload()
        {
            ...
            var pkt = _builder.BuildHandshakeInit(new byte[payloadSize], sealedApiKey: true);
        ...
        public void Build_PayloadBytes_AreCopiedCorrectly()
        {
            ...
            var pkt = _builder.BuildHandshakeInit(payload, sealedApiKey: true);
        ...
        public void BuildReconnectInit_WithProof_AppendsProofAfterToken()
        {
            ...
            for (int i = 0; i < 32; i++) commitment[i] = (byte)(0x80 + i);

            var pkt = _builder.BuildReconnectInit(token, commitment, proof);
            ...
            // The key commitment at payload[2+tokenLen..+32] — the fresh flow's
            // envelope names the key Round 2 must offer, and the reconnect init
            // carries it for the same reason, in cleartext.
            for (int i = 0; i < 32; i++)
                Assert.AreEqual(commitment[i], pkt[payloadOffset + 2 + tokenLen + i],
                    $"commitment byte mismatch at index {i}");

            // Proof bytes follow the commitment: payload[2+tokenLen+32..+32]
            for (int i = 0; i < 32; i++)
                Assert.AreEqual(proof[i], pkt[payloadOffset + 2 + tokenLen + 32 + i],
        ...
        public void BuildReconnectInit_WithoutProof_HasNoProofBytes()
        {
            ...
            var pkt = _builder.BuildReconnectInitWithoutProof(token, new byte[32]);

            var tokenBytes = System.Text.Encoding.UTF8.GetBytes(token);
            // Payload = 2 (len) + tokenLen + 32 (key commitment); no proof bytes
            Assert.AreEqual(13 + 2 + tokenBytes.Length + 32, pkt.Length,
                "packet without proof carries the commitment and no proof bytes");
        }
        ...
        public void BuildReconnectInit_WithProof_WrongProofLength_Throws()
        {
            Assert.Throws<ArgumentException>(() =>
                _builder.BuildReconnectInit("tok", new byte[32], new byte[16]));
            Assert.Throws<ArgumentException>(() =>
                _builder.BuildReconnectInit("tok", new byte[32], new byte[33]));
        }
