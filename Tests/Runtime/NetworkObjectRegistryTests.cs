...
namespace RTMPE.Tests
{
    ...
    public class NetworkObjectRegistryTests
    {
        ...
        public void Register_ExceptionInDespawn_DepthCounterRestored()
        {
            ...
            // The eviction runs through SpawnLifecycleOps.UnspawnAll with a fault
            // reporter, so a throwing OnNetworkDespawn is LOGGED as an exception
            // and never propagates out of Register — the same isolation every
            // other fan-out in the registry has. What this case holds is the
            // try/finally beneath it: the depth counter is restored either way.
            LogAssert.Expect(LogType.Exception, new System.Text.RegularExpressions.Regex("InvalidOperationException"));
            Assert.DoesNotThrow(() => _registry.Register(repl));
        ...
        // ── The engine semantics OBJECT-RV-01 rests on ────────────────────────
        //
        // `OBJECT-RV-01` says that destroying ONE NetworkBehaviour deregisters
        // the whole networked object, because OnDestroy is a component callback
        // and SpawnManager reconciles the object from it unconditionally.
        //
        // Every check on that outside this file is a source rule or a stubbed
        // unit test, and neither can answer the question the finding turns on:
        // does Unity raise OnDestroy on a component destroyed while its
        // GameObject lives on? That is engine behaviour, so it is asserted here,
        // where a real engine is running, instead of assumed in a comment.
        //
        // ⛔ These establish the FINDING, not a repair. The finding is open: a
        // repair that hands the anchor to a surviving sibling was attempted and
        // withdrawn, because it depends on a SECOND engine semantic these tests
        // do not settle — whether a component is already fake-null when a
        // sibling's OnDestroy runs during a whole-GameObject teardown. If it is
        // not, every component re-anchors and nothing ever reconciles, which
        // leaks a live-count slot on every teardown rather than on the rare
        // single-component destroy. Settle that here before trying again.

        [Test]
        public void Unity_DestroyingOneBehaviour_LeavesTheGameObjectAndItsSiblings()
        {
            var go = new GameObject("two-behaviours");
            _created.Add(go);

            var first  = go.AddComponent<ConcreteNB>();
            var second = go.AddComponent<ConcreteNB>();
            first.Initialize(70, "p1");
            second.Initialize(70, "p1");

            Object.DestroyImmediate(second);

            Assert.IsTrue(go != null, "the GameObject must outlive the component");
            Assert.IsTrue(first != null, "the sibling must outlive the component");
            Assert.IsTrue(second == null, "the destroyed component must read as null");
        }

        [Test]
        public void Unity_RaisesOnDestroy_ForAComponentDestroyedOnItsOwn()
        {
            // The load-bearing half. If this ever fails, OBJECT-RV-01 is not a
            // defect and the reconciliation on OnDestroy is safe as it stands.
            var go = new GameObject("one-behaviour-of-two");
            _created.Add(go);

            var keep = go.AddComponent<ConcreteNB>();
            var doomed = go.AddComponent<DestroyCountingNB>();
            keep.Initialize(73, "p1");

            Assert.AreEqual(0, doomed.OnDestroyCalls);
            Object.DestroyImmediate(doomed);

            Assert.AreEqual(1, DestroyCountingNB.LastOnDestroyCalls,
                "Unity did not raise OnDestroy for a component destroyed on its "
                + "own, so OBJECT-RV-01's premise does not hold on this engine");
        }

    }

    /// <summary>
    /// Counts its own <c>OnDestroy</c>. A static mirror is kept because the
    /// instance is unreadable once Unity has destroyed it.
    /// </summary>
    public class DestroyCountingNB : NetworkBehaviour
    {
        public static int LastOnDestroyCalls;
        public int OnDestroyCalls;

        protected override void OnDestroy()
        {
            OnDestroyCalls++;
            LastOnDestroyCalls = OnDestroyCalls;
            base.OnDestroy();
        }
    }
}
