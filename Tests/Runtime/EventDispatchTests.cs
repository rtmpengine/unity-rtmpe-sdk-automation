...
namespace RTMPE.Tests
{
    ...
    public class EventDispatchTests
    {
        ...
        public void SetUp()
        {
            // The subscriber-throw report is gated once per second across the
            // process; a case that expects it must not inherit the previous
            // case's spend.
            NetworkManager.ResetSubscriberThrowWarningForTests();
        ...
        public void OnConnected_MultipleThrowers_AllSurvived()
        {
            ...
            // One pair, not two: the report is gated to once per second per
            // process (WarnGate), so the second thrower in the same raise is
            // suppressed by design. Every subscriber still runs — that is the
            // property, and `counter` holds it.
