    ...
    public class RpcPolishTests
    {
        ...
        public void NetworkVariableList_FullSync_OversizeCount_Rejected()
        {
            ...
            try
            {
                ...
                var src = new NetworkVariableListInt(owner, "v0");
                // Pre-populate the receiver to ensure rejection truly DROPS the
                // payload (rather than silently clearing).
                //
                // A distinct name: both live on one behaviour, which refuses two
                // variables deriving one identity. The payload below is fed to
                // `dst.Deserialize` directly, so the id is never read.
                var dst = new NetworkVariableListInt(owner, "v0-readback");
                dst.Add(7);
        ...
        public void NetworkVariableList_FullSync_AtCap_Accepted()
        {
            ...
            try
            {
                ...
                var dst = new NetworkVariableListInt(owner, "v0");
