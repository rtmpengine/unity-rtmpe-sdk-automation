...
//          → bytes transmitted
//              → target: ApplyFromWire(Deserialize(BinaryReader)) — notifies, never dirties
...
namespace RTMPE.Tests
{
    ...
    public class ReceivePipelineIntegrationTests
    {
        ...
        {
            if (_interpGo != null) { UnityEngine.Object.DestroyImmediate(_interpGo); _interpGo = null; }
            if (_nmGo     != null) { UnityEngine.Object.DestroyImmediate(_nmGo);     _nmGo     = null; }
        }
    ...
    public class NetworkVariablePipelineTests
    {
        ...
        // The receiving end, because a round-trip has two and one behaviour
        // cannot hold two variables under one id.
        private GameObject     _receiverGo;
        private VarStubNB      _receiver;
        ...
        public void SetUp()
        {
            ...
            // ⚠️ Two gates gate a write. The setter returns on
            // `!Owner.IsSpawned`, and then again on `WriteWouldNotBeSent` —
            // which is `IsSpawned && IsOwnedByAnotherPlayer`, so spawning the
            // owner is what arms it. Without a seat named here the owner id
            // matches nothing this client claims, the object reads as somebody
            // else's, and every assignment below is refused a second time.
            ...
            _receiver   = _receiverGo.AddComponent<VarStubNB>();
        ...
        public void TearDown()
        {
            if (_receiverGo != null) { UnityEngine.Object.DestroyImmediate(_receiverGo); _receiverGo = null; }
            if (_ownerGo    != null) { UnityEngine.Object.DestroyImmediate(_ownerGo);    _ownerGo    = null; }
            if (_nmGo       != null) { UnityEngine.Object.DestroyImmediate(_nmGo);       _nmGo       = null; }
        }
        ...
        // dst is the "receiver" side (ApplyFromWire — notifies, never dirties).
        ...
        {
            var src = new NetworkVariableInt(_owner, "v0", 12345);
        ...
        {
            var srcA = new NetworkVariableFloat(_owner, "v0", 1.5f);
            var srcB = new NetworkVariableFloat(_owner, "v1", 9.75f);
            var dstA = new NetworkVariableFloat(_receiver, "v0", 0f);
            var dstB = new NetworkVariableFloat(_receiver, "v1", 0f);
        ...
        {
            var v = new NetworkVariableInt(_owner, "v0", 0);
            v.Value = 42;
        ...
        {
            var src = new NetworkVariableString(_owner, "v0", "initial");
            var dst = new NetworkVariableString(_receiver, "v0", "other");
        ...
        public void Vector3Var_PipelineRoundTrip_AllComponentsMatch()
        {
            ...
            var src = new NetworkVariableVector3(_owner, "v0", pos);
        ...
        public void ReceiverSide_SetViaSetter_FiresEvent()
        {
            ...
            // (Deserialize notifies too, since v8.  This case is about the SETTER
            // path that game code uses when it owns the variable locally.)
            var v = new NetworkVariableInt(_owner, "v0", 0);
            int callCount = 0;
    ...
    public class SyncPerformanceTests
    {
        ...
        {
            // ⚠️ Every GameObject is created before the try and released in the
            // finally. A leaked NetworkManager is not an untidy test: the next
            // fixture's Awake finds a singleton already claimed and destroys its
            // own manager instead, so a seat named there is never published and
            // the fixture after this one silently measures a refused write.
            var nmGo   = new GameObject("PerfNM");
            var goOwn  = new GameObject("PerfOwner");
            var goRecv = new GameObject("PerfReceiver");

            try
            {
                var manager = nmGo.AddComponent<NetworkManager>();
                manager.SetLocalPlayerStringId("perf");

                var stub = goOwn.AddComponent<PerfNB>();
                stub.Initialize(1UL, "perf");
                stub.SetSpawned(true);

                // Deserialize returns for an unspawned owner, and a second
                // variable under an id the behaviour already tracks throws out
                // of its own constructor — so the loop below measured neither
                // the serializer nor the deserializer before this.
                var recv = goRecv.AddComponent<PerfNB>();
                recv.Initialize(2UL, "perf");
                recv.SetSpawned(true);

                var src = new NetworkVariableInt(stub, "v0", 42);
                var dst = new NetworkVariableInt(recv, "v0", 0);
            ...
            {
                UnityEngine.Object.DestroyImmediate(goRecv);
                UnityEngine.Object.DestroyImmediate(goOwn);
