...
namespace RTMPE.Tests
{
   ...
   /// These tests act as a static contract guard: if any constant diverges from the
    ...
    public class ProtocolConstantsTests
    {
        ...
        [Test]
        [Description("Protocol version byte must be 5 — must match Rust VERSION constant")]
        public void Version_Matches_GatewayConstant()
        {
            Assert.AreEqual(5, PacketProtocol.VERSION,
                "VERSION must equal 5 (Rust: pub const VERSION: u8 = 5)");
        }
        ...
        public void PacketType_Count_MatchesGatewayInventory()
        {
            ...
            Assert.AreEqual(43, values.Length,
                "Exactly 43 PacketType members must exist (Rust gateway header.rs " +
                "PacketType::ALL declares 43: the 41 a client may send, which TryFrom " +
                "accepts, plus the two outbound-only answers HandshakeError 0x0B and " +
                "SpawnRejected 0x32)");
            // ⚠️ This assertion read 41 while the enum declared 42, and then 43.
            // Nothing noticed: CI has no Unity licence, so the in-package runner
            // never executes, and check-unity-inpackage-tests.sh counts LINES.
            // The xunit counterpart that CI does run is
            // tests/unit/unity-sdk-protocol/PacketTypeParityTests.cs, and it is
            // the one to change first — this copy is a shipped assertion about
            // the same contract and drifts silently behind it.
        }
