...
namespace RTMPE.Tests
{
    ...
    public class RpcPacketBuilderTests
    {
        ...
        [Test]
        [Description("BuildRequest at the sendable ceiling succeeds. The bound is " +
                     "MaxSendablePayloadBytes — what is left of one datagram after the " +
                     "packet and RPC headers — NOT RpcLimits.MaxPayloadBytes, which is the " +
                     "bound on what may be RECEIVED. These asserted 4096 until 2026-08-23, " +
                     "roughly 3.6x what could ever be framed.")]
        public void BuildRequest_MaxSendablePayload_Succeeds()
        {
            var payload = new byte[RpcPacketBuilder.MaxSendablePayloadBytes];

            Assert.DoesNotThrow(() => RpcPacketBuilder.BuildRequest(0, 1UL, 0, payload));
        }

        [Test]
        [Description("One byte past the sendable ceiling throws ArgumentException.")]
        public void BuildRequest_OversizedPayload_Throws()
        {
            var payload = new byte[RpcPacketBuilder.MaxSendablePayloadBytes + 1];

            Assert.Throws<ArgumentException>(
                () => RpcPacketBuilder.BuildRequest(0, 1UL, 0, payload));
        }

        [Test]
        [Description("The receive bound is larger than anything that can be sent — if it " +
                     "were not, the two constants would be answering the same question.")]
        public void TheReceiveBoundExceedsTheSendableOne()
        {
            Assert.Less(RpcPacketBuilder.MaxSendablePayloadBytes, RpcLimits.MaxPayloadBytes);
        }
