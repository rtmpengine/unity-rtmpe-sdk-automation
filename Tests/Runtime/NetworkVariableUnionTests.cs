...
namespace RTMPE.Tests
{
    ...
    {
        // The widths the schema declares — `object_id: ulong`, `variable_id: uint`.
        // A narrower constant here compiles (both widen implicitly) and then
        // compares against the accessor's own width, which is the shape that let
        // seven `byte variableId` overloads sit in the encoder unnoticed.
        private const ulong ObjectId    = 17UL;
        private const uint  VariableId  = 3u;
        ...
        public void VerifiedFlatBuffer_RejectsMismatchedVerifierAndRoot()
        {
            ...
            //
            // Stamped with NetworkVariableUpdateV2's own file identifier so
            // that the discriminant is the only thing left that can refuse it:
            // finished bare, the identifier comparison speaks first and this
            // case would pass with the discriminant deleted.
            ...
            b.Finish(end.Value, VerifiedFlatBuffer.NetworkVariableUpdateV2Identifier);
            ...
            Func<FlatBufferBuilder, Offset<FbNetworkVariableUpdateV2>> build)
        {
            ...
            // Finished the way the SDK finishes it, which writes the file
            // identifier. A bare builder.Finish produces a buffer no encoder in
            // this tree emits, and the receive gate is entitled to refuse it —
            // so a fixture built that way tests a producer that does not exist.
            NetworkVariableEncoding.Finish(b, offset);
