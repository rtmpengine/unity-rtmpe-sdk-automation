...
namespace RTMPE.Tests
{
    ...
    public class NetworkTransformTests
    {
        ...
        [Test]
        [Description("OwnerTeleportTo carries the velocity-cap baseline to the destination, so the jump itself is not metered as travel.")]
        public void OwnerTeleportTo_CarriesTheBaselineToTheDestination()
        {
            ...
            // A candidate at the new position passes unchanged: the baseline
            // travelled with the owner, so the displacement between them is
            // zero.  Were the baseline left at the departure point, ten
            // kilometres would be charged against the cap here.
