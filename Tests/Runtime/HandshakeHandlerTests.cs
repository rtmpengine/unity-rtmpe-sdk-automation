    ...
    public class HandshakeHandlerTests
    {
        ...
        public void HandshakeHandler_ValidateChallenge_ReturnsFalseForNullPayload()
        {
            ...
            Assert.IsFalse(h.ValidateChallenge(
                null, roundOnePayload: null, HandshakeFlow.Reconnect, out _, out _));
        }
       ...
       /// Uses Curve25519 and HkdfSha256 directly (InternalsVisibleTo is required).
        ...
        [Test]
        [Category("KeyHex")]
        public void KeyHex_Decode32_DecodesCorrectly()
        {
            string hex = "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f";
            var key = KeyHex.Decode32(hex);
            Assert.AreEqual(32, key.Length);
            for (int i = 0; i < 32; i++)
                Assert.AreEqual((byte)i, key[i], $"byte[{i}]");
        }

        [Test]
        [Category("KeyHex")]
        public void KeyHex_Decode32_ThrowsOnShortKey()
        {
            Assert.Throws<ArgumentException>(() => KeyHex.Decode32("0011223344"));
        }

        [Test]
        [Category("SealedApiKeyCipher")]
        public void SealedApiKeyCipher_Seal_ProducesNonDeterministicOutput()
        {
            var (_, recipientPub) = Curve25519.GenerateKeyPair();

            var box1 = SealedApiKeyCipher.Seal(recipientPub, "my-api-key", CommitmentStandIn);
            var box2 = SealedApiKeyCipher.Seal(recipientPub, "my-api-key", CommitmentStandIn);

            // Each call draws a fresh ephemeral keypair → boxes must differ.
            Assert.IsFalse(BytesEqual(box1, box2),
                "Each Seal() call must produce a different box (fresh ephemeral).");
        }

        [Test]
        [Category("SealedApiKeyCipher")]
        public void SealedApiKeyCipher_Seal_HasTheWireShape()
        {
            var (_, recipientPub) = Curve25519.GenerateKeyPair();
            var apiKey = "hello";
            var keyBytes = Encoding.UTF8.GetBytes(apiKey);

            // [eph_pub:32][key_len:2][apiKey:N][commitment:32][Poly1305Tag:16]
            var box = SealedApiKeyCipher.Seal(recipientPub, apiKey, CommitmentStandIn);
            int expected = 32 + 2 + keyBytes.Length + SealedApiKeyCipher.PublicKeyLen + 16;
            Assert.AreEqual(expected, box.Length,
                $"Sealed box must be eph(32) + len(2) + key({keyBytes.Length}) + commitment(32) + tag(16) = {expected} bytes.");
        }
