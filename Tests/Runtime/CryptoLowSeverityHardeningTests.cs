...
//
//  • L SealedApiKeyCipher.Seal zeros the API-key UTF-8 buffer it allocated.
...
//          documented inline.
//  • L BuildReconnectInit(token, key, null) throws ArgumentNullException.
...
namespace RTMPE.Tests
{
    ...
    {
        /// <summary>A distinctive stand-in for the X25519 public key a client
        /// commits to in <c>HandshakeInit</c>.  Every byte is non-zero, so a
        /// commitment that is silently zero-filled shows up as a difference
        /// rather than as a plausible value.</summary>
        private static byte[] CommitmentStandIn
            => System.Linq.Enumerable.Range(1, 32).Select(i => (byte)i).ToArray();
        ...
        public void BuildReconnectInit_NullProof_Throws()
        {
            ...
            Assert.Throws<ArgumentNullException>(
                () => builder.BuildReconnectInit("tok", new byte[32], null));
        }
        ...
        {
            var builder = new PacketBuilder();
            var pkt = builder.BuildReconnectInitWithoutProof("tok", new byte[32]);
            Assert.IsNotNull(pkt);
            Assert.AreEqual(13 + 2 + 3 + 32, pkt.Length,
                "without-proof variant carries the 32-byte key commitment and no 32-byte proof block");
        }
        ...
        public void ComputeReconnectProof_ProducesDeterministic32ByteHmac()
        {
            ...
            var p1 = PacketBuilder.ComputeReconnectProof("tok-xyz", new byte[32], key);
            var p2 = PacketBuilder.ComputeReconnectProof("tok-xyz", new byte[32], key);
        ...
        public void ComputeReconnectProof_RejectsInvalidKey()
        {
            Assert.Throws<ArgumentException>(
                () => PacketBuilder.ComputeReconnectProof("tok", new byte[32], new byte[16]));
            Assert.Throws<ArgumentException>(
                () => PacketBuilder.ComputeReconnectProof("tok", new byte[32], null));
        }
        ...
        // Build a 13-byte RTMPE header (magic 0x5254 "RT", current version —
        // the line below writes `PacketProtocol.VERSION`). ⚠️ This comment used
        // to say "RTMPE-v3 … version 3", which the code has never done, and it
        // was the tree's ONLY mention of a protocol version 3 — cited in
        // earnest by a later comment as evidence that a legacy gateway stamped
        // one.
        ...
        // ── The sealed box zeroizes its plaintext intermediate ─────────
        //
        // The internal `plaintext` buffer is a local dropped to GC and cannot
        // be observed after Seal returns.  What can be observed is the output:
        // if the wipe left a stray copy spliced in, or the AEAD failed to
        // cover the key, the canary appears verbatim in the box.

        [Test]
        [Category("SealedApiKeyCipher")]
        public void Seal_DoesNotLeakApiKeyPlaintextInOutput()
        {
            var (_, recipientPub) = Curve25519.GenerateKeyPair();
            const string apiKey = "RTMPE-test-xxxxxxxxxxxxxxxxxxxxxxx";

            byte[] box = SealedApiKeyCipher.Seal(recipientPub, apiKey, CommitmentStandIn);

            // The box MUST NOT contain the plaintext API key as a contiguous
            // run of bytes.  This is a sanity check for both the AEAD doing its
            // job AND for the zeroize cleanup not leaving a stray copy spliced
            // into the output.
            var keyBytes = System.Text.Encoding.UTF8.GetBytes(apiKey);
            Assert.IsFalse(
                ContainsSubsequence(box, keyBytes),
                "the sealed box must not contain the plaintext API key byte sequence");
        }
