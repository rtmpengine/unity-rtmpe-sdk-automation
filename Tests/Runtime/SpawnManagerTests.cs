...
namespace RTMPE.Tests
{
    ...
    public class SpawnManagerTests
    {
        ...
        [Test]
        [Description("Spawn with unregistered prefab returns null and logs a warning.")]
        public void Spawn_UnregisteredPrefab_ReturnsNullAndLogsError()
        {
            // No prefab registered for PREFAB_ID. The runtime reports this as a
            // warning — a missing registration is recoverable by registering.
            LogAssert.Expect(LogType.Warning, new System.Text.RegularExpressions.Regex("not registered"));
        ...
        [Test]
        [Description("Spawn with an explicit owner that IS the local player uses it.")]
        public void Spawn_ExplicitOwner_MatchingLocalPlayer_Overrides()
        {
            RegisterDefaultPrefab();
            _manager.SetLocalPlayerStringId("default-player");

            var nb = SpawnDefault(owner: "default-player");

            Assert.AreEqual("default-player", nb.OwnerPlayerId);
        }

        [Test]
        [Description(
            "Spawn under ANOTHER player's identity is refused and creates nothing: " +
            "the gateway relays a spawn only under the spawner's own identity, so the " +
            "object would have existed on this client alone.")]
        public void Spawn_ExplicitOwner_NamingAnotherPlayer_IsRefused()
        {
            RegisterDefaultPrefab();
            _manager.SetLocalPlayerStringId("default-player");
            LogAssert.Expect(LogType.Error,
                new System.Text.RegularExpressions.Regex("Spawn refused"));

            var nb = SpawnDefault(owner: "explicit-owner");

            Assert.IsNull(nb, "Spawn must refuse an owner claim naming another player.");
            Assert.AreEqual(0, _registry.GetAll().Count,
                "A refused spawn must leave nothing behind locally.");
        }

        [Test]
        [Description(
            "A client that has not yet been told its own player id refuses nothing: " +
            "such a spawn is local-only whatever this side decides, so returning null " +
            "would take away a usable local object rather than prevent a divergence.")]
        public void Spawn_ExplicitOwner_BeforeTheLocalIdIsKnown_IsAdmitted()
        {
            RegisterDefaultPrefab();
        ...
        [Test]
        [Description(
            "An EMPTY owner reads as 'none specified' and resolves to the local " +
            "player: the gateway refuses an in-room spawn that claims no owner at " +
            "all, and only a modified client emits one.")]
        public void Spawn_EmptyOwner_ResolvesToTheLocalPlayer()
        {
            RegisterDefaultPrefab();
            _manager.SetLocalPlayerStringId("default-player");

            var nb = SpawnDefault(owner: string.Empty);
        ...
        [Test]
        [Description(
            "Despawning ANOTHER player's object is refused and destroys nothing: " +
            "the gateway relays a despawn only from the object's owner and answers " +
            "a refusal with no packet, so the object would have vanished here and " +
            "stood for everyone else.")]
        public void Despawn_ObjectOwnedByAnotherPlayer_IsRefused()
        {
            RegisterDefaultPrefab();
            var nb = SpawnDefault(owner: "alice");
            var id = nb.NetworkObjectId;
            _manager.SetLocalPlayerStringId("bob");
            LogAssert.Expect(LogType.Error,
                new System.Text.RegularExpressions.Regex("Despawn refused"));

            _spawnManager.Despawn(id);

            Assert.IsTrue(nb.IsSpawned, "A refused despawn must leave the object alone.");
            Assert.IsNotNull(_registry.Get(id));
        }

        [Test]
        [Description("TryDespawn reports the refusal instead of logging it.")]
        public void TryDespawn_ObjectOwnedByAnotherPlayer_ReturnsFalse()
        {
            RegisterDefaultPrefab();
            var nb = SpawnDefault(owner: "alice");
            _manager.SetLocalPlayerStringId("bob");

            Assert.IsFalse(_spawnManager.CanDespawn(nb.NetworkObjectId));
            Assert.IsFalse(_spawnManager.TryDespawn(nb.NetworkObjectId));
            Assert.IsTrue(nb.IsSpawned);
        }
        ...
        [Description(
            "A client that has not yet been told its own player id refuses nothing: " +
            "no spawn of its own has been relayed, so no peer holds the object and " +
            "the local teardown is the whole operation.")]
        public void Despawn_BeforeTheLocalIdIsKnown_IsAdmitted()
        {
            RegisterDefaultPrefab();
            var nb = SpawnDefault(owner: "alice");

            Assert.IsTrue(_spawnManager.CanDespawn(nb.NetworkObjectId));
            _spawnManager.Despawn(nb.NetworkObjectId);
