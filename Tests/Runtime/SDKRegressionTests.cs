...
namespace RTMPE.Tests
{
    ...
    public class NetworkVariableFlushTests
    {
        ...
        {
            var nvi = new NetworkVariableInt(_owner, "v0", initialValue: 10);
        ...
        {
            var nvi = new NetworkVariableInt(_owner, "v0", initialValue: 0);
        ...
        {
            var nvi = new NetworkVariableInt(_owner, "v0", initialValue: 5);
            nvi.Value = 42;
            Assert.IsTrue(nvi.IsDirty);
        ...
        public void FlushDirtyVariables_NotOwner_NoSend()
        {
            ...
            var nvi = new NetworkVariableInt(nb2, "v0", initialValue: 0);
            nvi.Value = 123;
            Assert.IsTrue(nvi.IsDirty);
        ...
        [Test]
        [Description("ApplyVariableUpdate deserializes an inbound update for the matching identity.")]
        public void ApplyVariableUpdate_SetsValueFromBinaryReader()
        {
            var nvi = new NetworkVariableInt(_owner, "v7", initialValue: 0);
            ...
            _owner.ApplyVariableUpdate(nvi.VariableId, reader);

            Assert.AreEqual(42, nvi.Value);
        }

        [Test]
        [Description("ApplyVariableUpdate ignores an identity no variable holds.")]
        public void ApplyVariableUpdate_WrongVarId_Ignores()
        {
            var nvi = new NetworkVariableInt(_owner, "v5", initialValue: 100);
            ...
            // One bit away from the variable's own identity: different by
            // construction rather than by a literal that may one day be it.
            _owner.ApplyVariableUpdate(nvi.VariableId ^ 1u, reader);

            Assert.AreEqual(100, nvi.Value, "Value must remain unchanged for an identity it does not hold.");
        }
        ...
        {
            var nvi1 = new NetworkVariableInt(_owner,   "v0", initialValue: 0);
            var nvi2 = new NetworkVariableFloat(_owner,  "v1", initialValue: 0f);
        ...
        public void FlushDirtyVariables_LongString_DoesNotThrow()
        {
            ...
            var nvs = new NetworkVariableString(_owner, "v0", initialValue: "");
        ...
        {
            // 300 ASCII chars: SerializeWithId emits [var_id:4][value_len:2][ushort_prefix:2][utf8:300]
            // = 308 bytes per variable.  FlushDirtyVariables prepends
            // [object_id:8][tick:4][count:1] = 13 bytes.
            // Expected total payload = 13 + 308 = 321 bytes.
            var nvs = new NetworkVariableString(_owner, "v5", initialValue: "");
            ...
            Assert.AreEqual(321, captured.Length,
                "Payload: 8 (object_id) + 4 (tick) + 1 (count) + 4 (var_id) + 2 (value_len) + 2 (ushort str prefix) + 300 (utf8) = 321.");
