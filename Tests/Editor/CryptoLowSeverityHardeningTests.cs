...
namespace RTMPE.Tests.Editor
{
    ...
    {
        // ⛔ Scoped, because that is the slot production writes. Held bare,
        // SetUp snapshotted a slot nothing uses while the store wrote the
        // developer's real per-project key, and TearDown restored the wrong
        // one — a green run destroying a stored credential.
        ...
        {
            const string apiKey = "rtmpe_live_xxxxxxxxxxxxxxxxxxxxxxx";
            var blob     = EditorApiKeyStore.Encrypt(apiKey);
            var recovered = EditorApiKeyStore.Decrypt(blob);
            Assert.AreEqual(apiKey, recovered);
        }
        ...
        {
            const string apiKey = "rtmpe_test_nonce_freshness_var_a";
