...
namespace RTMPE.Rpc
{
    ...
    {
        /// <summary>
        /// Largest Enhanced-RPC parameter block that can actually be sent: what
        /// is left of one datagram's application payload after this protocol's
        /// own request header.
        /// </summary>
        /// <remarks>
        /// 🔑 Derived, never written down. The declared limit and the enforced
        /// limit have to be one number, and the only way to keep them so is to
        /// compute one from the other — a literal goes stale the first time the
        /// datagram budget, the packet header or the sub-header reservation
        /// moves, and goes stale silently, because nothing downstream reads it.
        ///
        /// <para>⛔ Here rather than on <see cref="RpcLimits"/>, which is
        /// deliberately dependency-free so the analyzer shard can link it alone
        /// to pin the conversion toolchain's independent copy of the reserved
        /// ids. This is the layer that already depends on both: its own output
        /// is documented as "ready for PacketBuilder.Build()".</para>
        /// </remarks>
        ...
        /// exceeds <see cref="MaxSendablePayloadBytes"/>, or
