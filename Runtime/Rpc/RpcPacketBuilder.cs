...
namespace RTMPE.Rpc
{
    ...
    {
        /// <summary>
        /// Largest legacy-RPC payload that can actually be sent: what is left
        /// of one datagram's application payload after this protocol's request
        /// header.
        /// </summary>
        /// <remarks>
        /// 🔑 The same correction as
        /// <c>EnhancedRpcPacketBuilder.MaxSendablePayloadBytes</c>, on the path
        /// that carries the SDK's own built-in method ids. Enforcing
        /// <see cref="RpcLimits.MaxPayloadBytes"/> here declared legal every
        /// payload up to 4096 bytes, of which only the first
        /// <see cref="MaxSendablePayloadBytes"/> can be framed — and the
        /// refusal then came from <c>PacketBuilder.Build</c>, one layer down.
        ///
        /// <para>⛔ Derived, never written down: a literal goes stale the first
        /// time the datagram budget, the packet header or the sub-header
        /// reservation moves, and goes stale silently.</para>
        /// </remarks>
        ...
        /// <see cref="MaxSendablePayloadBytes"/>.
        /// </exception>
