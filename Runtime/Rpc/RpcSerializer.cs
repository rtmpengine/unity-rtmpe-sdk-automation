...
namespace RTMPE.Rpc
{
    ...
    public static class RpcSerializer
    {
        ...
        // Emission gates for the three type-rejection diagnostics below.  Static,
        // because the loop that reaches them is bounded by a wire byte and a
        // per-call gate would bound nothing.  One each rather than one shared,
        // because the three name different repairs — register the type, register
        // it generically so IL2CPP keeps a factory, implement the interface — and
        // a project sitting on any one of them permanently would otherwise spend
        // the gate every second and hide the other two indefinitely.
        //
        // Only the first renders a name the sender chose; past it the lookup has
        // succeeded and the name is a registered type's own, from a closed set.
        // The sanitiser is applied to all three anyway, because what makes the
        // later two safe is a property of the registry rather than of this code,
        // and a future registration path that admits a wire-derived name would
        // otherwise arrive here unguarded.
        private static long _lastUnregisteredTypeWarnTicks;
        private static long _lastMissingFactoryWarnTicks;
        private static long _lastNotSerializableWarnTicks;
        ...
        public static int WriteParam(object value, byte[] buf, int offset)
        {
            ...
            if (value is string str)
            {
                ...
                // surface closed for the API-key envelope in M19-CRYPTO-01.
        ...
        public static object ReadParam(byte[] data, ref int offset)
        {
            ...
            switch (typeId)
            {
                ...
                case RpcTypeId.INetworkSerializable:
                {
                    ...
                    if (concrete == null)
                    {
                        if (WarnGate.ShouldEmit(ref _lastUnregisteredTypeWarnTicks))
                        {
                            Debug.LogWarning(
                                $"[RTMPE] RpcSerializer: rejected unregistered " +
                                $"INetworkSerializable type '{UntrustedLogText.Sanitise(typeName)}' on inbound RPC. " +
                    ...
                    if (instance == null)
                    {
                        if (WarnGate.ShouldEmit(ref _lastMissingFactoryWarnTicks))
                        {
                            Debug.LogWarning(
                                $"[RTMPE] RpcSerializer: no IL2CPP-safe factory for '{UntrustedLogText.Sanitise(typeName)}'. " +
                    ...
                    {
                        if (WarnGate.ShouldEmit(ref _lastNotSerializableWarnTicks))
                        {
                            Debug.LogWarning(
                                $"[RTMPE] RpcSerializer: type '{UntrustedLogText.Sanitise(typeName)}' resolved but does not " +
