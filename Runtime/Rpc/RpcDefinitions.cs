...
namespace RTMPE.Rpc
{
    ...
    {
        /// <summary>
        /// Wire-validation bound on the parameter block of an Enhanced RPC
        /// (4 KiB) — what this client must be able to RECEIVE.
        /// </summary>
        /// <remarks>
        /// ⚠️ This is not what a caller may SEND, and reading it as such is the
        /// defect it used to be. What may be sent is
        /// <see cref="MaxSendablePayloadBytes"/>, which is smaller by a factor
        /// of nearly four: an outbound frame has to survive one datagram, and
        /// the builder that enforced only this limit declared legal a call that
        /// the packet builder one layer down then refused.
        ///
        /// <para>The same distinction, for the same reason, as
        /// <c>PacketBuilder.MaxPayloadBytes</c> against
        /// <c>PacketBuilder.MaxApplicationPayloadBytes</c>.</para>
        ///
        /// <para>⛔ The sendable ceiling is
        /// <c>EnhancedRpcPacketBuilder.MaxSendablePayloadBytes</c> and lives
        /// there rather than here on purpose: it is derived from the packet
        /// layer's datagram budget, and <b>this file is dependency-free</b> —
        /// the analyzer shard links it on its own to pin the conversion
        /// toolchain's independent copy of the reserved ids against it, which a
        /// single reference to another namespace would end.</para>
        /// </remarks>
