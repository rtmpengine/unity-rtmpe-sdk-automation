 ...
 */
/*
 * Modified by RTMPE. This file is not the upstream FlatBuffers source: it
 * carries receive-path hardening applied on top of it. The changes are listed,
 * file by file, in VENDOR.md in this directory. Notice given under section 4(b)
 * of the Apache License, Version 2.0, under which this file remains licensed.
 */
...
namespace Google.FlatBuffers
{
  ...
  public class Options
  {
    ...
    // Local hardening over upstream: the upstream default of 1_000_000 tables
    // is calibrated for arbitrarily large file-backed FlatBuffers, but every
    // RTMPE payload is bounded by the 16 KB UDP MTU. The explicit Options
    // constructed in VerifiedFlatBuffer.TryGetRoot already wires depth=32 and
    // tables=16384; this default is lowered to match so any future caller that
    // forgets to pass options does not silently fall back to the upstream cap
    // that admits adversarial table-bombs. The two constants must stay in
    // lockstep — see VerifiedFlatBuffer.MaxTablesPerBuffer.
    public const int DEFAULT_MAX_TABLES = 16384;
  ...
  public class Verifier
  {
    ...
    /// <remarks>
    /// An identifier is compared when one is supplied; null or empty skips the
    /// comparison, per VerifyBuffer's contract. The buffer must hold the four
    /// identifier bytes that follow the root offset AT <paramref name="startPos"/>,
    /// which is not zero for a size-prefixed buffer.
    /// <para>RTMPE local hardening — both clauses. See VENDOR.md.</para>
    /// </remarks>
    private bool CheckBufferFromStart(string identifier, uint startPos, VerifyTableAction verifyAction)
    {
      if ((identifier != null) &&
          (identifier.Length != 0) &&
          ((verifier_buffer.Length < (startPos + SIZE_U_OFFSET + FILE_IDENTIFIER_LENGTH)) || (!BufferHasIdentifier(verifier_buffer, startPos, identifier))))
      {
        return false;
      }
