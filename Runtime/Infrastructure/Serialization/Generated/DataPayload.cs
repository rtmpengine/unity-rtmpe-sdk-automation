// <auto-generated>
//  automatically generated by the FlatBuffers compiler, do not modify
// </auto-generated>

namespace RTMPE.Packets
{

using global::System;
using global::System.Collections.Generic;
using global::Google.FlatBuffers;

public struct DataPayload : IFlatbufferObject
{
  private Table __p;
  public ByteBuffer ByteBuffer { get { return __p.bb; } }
  public static void ValidateVersion() { FlatBufferConstants.FLATBUFFERS_25_12_19(); }
  public static DataPayload GetRootAsDataPayload(ByteBuffer _bb) { return GetRootAsDataPayload(_bb, new DataPayload()); }
  public static DataPayload GetRootAsDataPayload(ByteBuffer _bb, DataPayload obj) { return (obj.__assign(_bb.GetInt(_bb.Position) + _bb.Position, _bb)); }
  public void __init(int _i, ByteBuffer _bb) { __p = new Table(_i, _bb); }
  public DataPayload __assign(int _i, ByteBuffer _bb) { __init(_i, _bb); return this; }

  public uint TypeTag { get { int o = __p.__offset(4); return o != 0 ? __p.bb.GetUint(o + __p.bb_pos) : (uint)0; } }
  public byte Data(int j) { int o = __p.__offset(6); return o != 0 ? __p.bb.Get(__p.__vector(o) + j * 1) : (byte)0; }
  public int DataLength { get { int o = __p.__offset(6); return o != 0 ? __p.__vector_len(o) : 0; } }
#if ENABLE_SPAN_T
  public Span<byte> GetDataBytes() { return __p.__vector_as_span<byte>(6, 1); }
#else
  public ArraySegment<byte>? GetDataBytes() { return __p.__vector_as_arraysegment(6); }
#endif
  public byte[] GetDataArray() { return __p.__vector_as_array<byte>(6); }

  public static Offset<RTMPE.Packets.DataPayload> CreateDataPayload(FlatBufferBuilder builder,
      uint type_tag = 0,
      VectorOffset dataOffset = default(VectorOffset)) {
    builder.StartTable(2);
    DataPayload.AddData(builder, dataOffset);
    DataPayload.AddTypeTag(builder, type_tag);
    return DataPayload.EndDataPayload(builder);
  }

  public static void StartDataPayload(FlatBufferBuilder builder) { builder.StartTable(2); }
  public static void AddTypeTag(FlatBufferBuilder builder, uint typeTag) { builder.AddUint(0, typeTag, 0); }
  public static void AddData(FlatBufferBuilder builder, VectorOffset dataOffset) { builder.AddOffset(1, dataOffset.Value, 0); }
  public static VectorOffset CreateDataVector(FlatBufferBuilder builder, byte[] data) { builder.StartVector(1, data.Length, 1); for (int i = data.Length - 1; i >= 0; i--) builder.AddByte(data[i]); return builder.EndVector(); }
  public static VectorOffset CreateDataVectorBlock(FlatBufferBuilder builder, byte[] data) { builder.StartVector(1, data.Length, 1); builder.Add(data); return builder.EndVector(); }
  public static VectorOffset CreateDataVectorBlock(FlatBufferBuilder builder, ArraySegment<byte> data) { builder.StartVector(1, data.Count, 1); builder.Add(data); return builder.EndVector(); }
  public static VectorOffset CreateDataVectorBlock(FlatBufferBuilder builder, IntPtr dataPtr, int sizeInBytes) { builder.StartVector(1, sizeInBytes, 1); builder.Add<byte>(dataPtr, sizeInBytes); return builder.EndVector(); }
  public static void StartDataVector(FlatBufferBuilder builder, int numElems) { builder.StartVector(1, numElems, 1); }
  public static Offset<RTMPE.Packets.DataPayload> EndDataPayload(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<RTMPE.Packets.DataPayload>(o);
  }
  public DataPayloadT UnPack() {
    var _o = new DataPayloadT();
    this.UnPackTo(_o);
    return _o;
  }
  public void UnPackTo(DataPayloadT _o) {
    _o.TypeTag = this.TypeTag;
    _o.Data = new List<byte>();
    for (var _j = 0; _j < this.DataLength; ++_j) {_o.Data.Add(this.Data(_j));}
  }
  public static Offset<RTMPE.Packets.DataPayload> Pack(FlatBufferBuilder builder, DataPayloadT _o) {
    if (_o == null) return default(Offset<RTMPE.Packets.DataPayload>);
    var _data = default(VectorOffset);
    if (_o.Data != null) {
      var __data = _o.Data.ToArray();
      _data = CreateDataVector(builder, __data);
    }
    return CreateDataPayload(
      builder,
      _o.TypeTag,
      _data);
  }
}

public class DataPayloadT
{
  public uint TypeTag { get; set; }
  public List<byte> Data { get; set; }

  public DataPayloadT() {
    this.TypeTag = 0;
    this.Data = null;
  }
}


static public class DataPayloadVerify
{
  static public bool Verify(Google.FlatBuffers.Verifier verifier, uint tablePos)
  {
    return verifier.VerifyTableStart(tablePos)
      && verifier.VerifyField(tablePos, 4 /*TypeTag*/, 4 /*uint*/, 4, false)
      && verifier.VerifyVectorOfData(tablePos, 6 /*Data*/, 1 /*byte*/, false)
      && verifier.VerifyTableEnd(tablePos);
  }
}

}
