...
//                     builder, objectId: 7, variableId: someVariable.VariableId,
//                     value: 42, timestampUs: 0);
//
// ⚠️ The two identity parameters are `ulong` and `uint` because the schema says
// so (`states.fbs`: `object_id: ulong`, `variable_id: uint`), and a narrower
// parameter here is not a smaller number — it is a DIFFERENT one. A variable's
// identity is an FNV-1a-32 hash of its declaring type and member name, so it
// uses the whole 32-bit range; these took `byte`, which no real identity fits,
// and C# would have narrowed each caller's id silently at the call site rather
// than at the wire, where nothing downstream could tell.
//   NetworkVariableEncoding.Finish(builder, offset);
//   byte[] wire = builder.SizedByteArray();
//
// Finish, and never builder.Finish: the buffer must carry the schema's file
// identifier, and a plain Finish omits it. The gateway refuses a buffer without
// it before it verifies anything else.
//
// Read path:
//
...
namespace RTMPE.Infrastructure.Serialization
{
    ...
    {
        /// <summary>
        /// Complete a buffer built by one of the Create* helpers above.
        /// </summary>
        /// <remarks>
        /// Wraps the generated finisher so callers never reach for
        /// <c>builder.Finish(offset.Value)</c>, which produces a byte-identical
        /// table with no file identifier — and the gateway refuses a buffer
        /// without the identifier before it verifies anything else. The
        /// identifier is the only part of the wire that states which table the
        /// sender believed it was writing; the structural verifier checks shape,
        /// and a shape this table accepts is exactly what a neighbouring table
        /// can produce.
        /// </remarks>
        public static void Finish(FlatBufferBuilder builder, Offset<NetworkVariableUpdateV2> offset)
        {
            if (builder == null) throw new ArgumentNullException(nameof(builder));
            NetworkVariableUpdateV2.FinishNetworkVariableUpdateV2Buffer(builder, offset);
        }
