...
//  2. Runs the structural verifier with limits sized generously above any
//     datagram this SDK sends (deeply-nested or table-bombed payloads are
//     dropped, not unwound at decode time via NRE / IndexOutOfRange).
//     ⚠️ The figures below are a SIZING assumption for those limits, not a
//     bound anything enforces: outbound frames are held to
//     PacketBuilder.MaxApplicationPayloadBytes, and the string bound the
//     reader actually applies is Table.MaxStringBytes (4096).
...
//  4. Runs a per-type SEMANTIC pass (ValidatePayloadSemantics) over the
//     fields the SDK's own receive path consumes, so a caller that reads
//     those fields cannot meet a value this build refuses.
//  5. Returns a bool plus an out-root, so call sites stay branch-friendly
//     and can drop the packet without unwinding through an exception.
//
// ⛔ What it does NOT cover, and the reason this is written down: FlatBuffers
// decodes fields LAZILY. Every accessor on the returned root runs in the
// caller's frame, after TryGetRoot has returned and outside every catch in
// this file — so a field the semantic pass above does not itself read is
// validated when the CALLER reads it, and its refusal is the caller's to
// handle. `Table.MaxStringBytes` (4096) refuses an oversized string exactly
// there, by throwing.  `TryReadField` below is that catch site, for callers
// who want the same drop-and-log behaviour on their own field reads.
//
// Design notes:
//
...
namespace RTMPE.Infrastructure.Serialization
{
    ...
    public static class VerifiedFlatBuffer
    {
            ...
            string diagnosticTag = null)
        {
            ...
            // Boundary discriminant, first of two.  The (tableVerifier, TRoot)
            // pair the caller passes must be one of the known good
            // combinations: a mismatched pair (e.g. verifying as InputPayload
            // but materialising as StateSyncPayload) would otherwise produce a
            // "verified" but type-confused root and surface garbage values to
            // game-state code.
            //
            // This is a statement about the receiving code, not about the
            // bytes, which is why it is only the first of two. The second is
            // RootIdentifier below: the one table rooted from an untrusted
            // buffer now carries a file identifier on the wire, so the sender's
            // own statement of what it wrote is checked as well. An earlier
            // version of this comment said the schemas were deliberately
            // unstamped because stamping would break already-deployed
            // identifier-less writers — no such writer exists, and the encoder
            // that builds this table had no caller outside its own tests.
            ...
            try
            {
                ...
                ok = verifier.VerifyBuffer(
                    identifier: RootIdentifier<TRoot>(),
                    sizePrefixed: false,
        ...
        private static void ValidatePayloadSemantics<TRoot>(TRoot root)
        {
            switch (root)
            {
                ...
                //
                // HandshakePayload, HandshakeAckPayload:
                //   The legacy 0x01/0x02 handshake, the only handshake packets
                //   that are FlatBuffers on the wire. Carry fixed-size
                //   cryptographic byte vectors (keys, signatures, nonces) and
                //   short string tokens. Crypto keys are bounded to 32–64 bytes
                //   structurally; string tokens are short by protocol
                //   definition. No float fields; no unbounded vector
                //   amplification.
                //   The 0x05–0x08 tables are not listed here for the reason
                //   given at the root-binding allowlist below.
                //
        ...
        /// <summary>
        /// The 4-character FlatBuffers file identifier a buffer rooted as
        /// <typeparamref name="TRoot"/> must carry, or <c>null</c> when that
        /// table declares none.
        /// </summary>
        /// <remarks>
        /// Only <c>NetworkVariableUpdateV2</c> declares one, and deliberately:
        /// <c>flatc</c> emits the identifier-writing finisher for a schema's
        /// declared root type alone, and stamping a table nothing roots from
        /// untrusted input would cost four bytes a frame for no boundary.
        ///
        /// Returning <c>null</c> for the rest is not a hole — the caller still
        /// has the (verifier, root) binding check — but it is the reason that
        /// check stays: an identifier is a per-table statement and the binding
        /// covers every table at once.
        ///
        /// ⚠️ Spelled here as a literal, and it should not have to be: the C#
        /// generator emits the identifier only inside
        /// <c>NetworkVariableUpdateV2BufferHasIdentifier</c>, with no constant
        /// to read, while the Rust generator does emit one. So this is a second
        /// copy of a wire value — held to the schema, the generated Rust
        /// constant and the generated C# helper by
        /// <c>scripts/check-flatbuffers-file-identifier.sh</c>, because a copy
        /// nothing compares is a copy that drifts the day somebody edits one of
        /// the four.
        /// </remarks>
        private static string RootIdentifier<TRoot>()
        {
            return typeof(TRoot).FullName == "RTMPE.States.NetworkVariableUpdateV2"
                ? NetworkVariableUpdateV2Identifier
                : null;
        }

        /// <summary>
        /// The file identifier <c>NetworkVariableUpdateV2</c> buffers carry.
        /// Held to the schema and both generators by
        /// <c>scripts/check-flatbuffers-file-identifier.sh</c>.
        /// </summary>
        internal const string NetworkVariableUpdateV2Identifier = "RVU2";
        ...
        private static bool IsRegisteredRootBinding<TRoot>(TableVerifier tableVerifier)
        {
            ...
            switch (typeof(TRoot).FullName)
            {
                ...
                // HandshakeInitPayload / ChallengePayload /
                // HandshakeResponsePayload / SessionAckPayload are deliberately
                // absent.  Packets 0x05–0x08 are raw binary on the wire — the
                // `.fbs` tables describe those layouts but are never the
                // encoding — so a buffer reaching this verifier under one of
                // those roots means a caller has mistaken the schema for the
                // protocol.  Rejecting the root binding makes that boundary
        ...
        /// <summary>
        /// Read one or more fields off a verified root, converting any refusal
        /// into the same drop-and-log this file applies to a structural failure.
        /// </summary>
        /// <typeparam name="TValue">Whatever the read produces.</typeparam>
        /// <param name="read">The field read, or a small group of them.</param>
        /// <param name="value">The result on success; default on refusal.</param>
        /// <param name="diagnosticTag">Short tag for the rate-limited warning.</param>
        /// <returns><c>false</c> if the read was refused; drop the packet.</returns>
        /// <remarks>
        /// 🔑 This exists because the contract in this file's header used to
        /// claim it already did. FlatBuffers decodes lazily, so
        /// <see cref="TryGetRoot{TRoot}"/> cannot catch a field read that has
        /// not happened yet: everything the caller reads off the returned root
        /// runs in the CALLER's frame, and the hardened accessors throw there by
        /// design — <c>Table.MaxStringBytes</c> refuses a string above 4096
        /// bytes, and <c>SafeFlatBufferAccessors</c> refuses malformed UTF-8, an
        /// embedded NUL, a non-finite float and an undefined enum tag.
        ///
        /// <para>⛔ Not a substitute for the semantic pass. That one runs over
        /// the fields the SDK's own receive path consumes and is not optional;
        /// this is for the fields a caller reads that the SDK does not.</para>
        /// </remarks>
        public static bool TryReadField<TValue>(
            Func<TValue> read,
            out TValue value,
            string diagnosticTag = null)
        {
            ...
            {
                LogVerificationFailure(diagnosticTag, "field read refused: " + ex.GetType().Name);
                value = default;
                return false;
            }
        }
