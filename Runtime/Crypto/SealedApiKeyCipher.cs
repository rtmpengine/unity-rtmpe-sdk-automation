...
// as the pinned server key. Nothing secret is distributed, so a shipped game
// binary carries no gateway credential to extract — which is what a symmetric
// envelope could never avoid, and why it was retired.
//
...
//
//   plaintext = [api_key_len:2 LE][api_key:N][client_ephemeral_pub:32]  (identical to the
//                                                         legacy ApiKeyCipher
//                                                         framing, so the
//                                                         gateway parse is
...
namespace RTMPE.Crypto
{
    ...
    /// and the only API-key envelope the protocol defines.
    /// </summary>
    public static class SealedApiKeyCipher
    {
        ...
        /// </summary>
        public const int MinSealedLen = PublicKeyLen + 2 + PublicKeyLen + TagLen;
            ...
            byte[] clientEphemeralPublicKey)
        {
            ...
            if (clientEphemeralPublicKey == null
                || clientEphemeralPublicKey.Length != PublicKeyLen)
                throw new ArgumentException(
                    $"clientEphemeralPublicKey must be exactly {PublicKeyLen} bytes.",
            ...
            try
            {
                ...
                // [api_key_len:2 LE][api_key:N][client_ephemeral_pub:32] — the
                // trailing key is what tells the gateway which X25519 key belongs
                // to the principal presenting this credential.  A relay may copy
                // the sealed box but cannot restate its contents, so it cannot
                // have the gateway complete ECDH with a key of its own.
                plaintext = new byte[2 + keyBytes.Length + PublicKeyLen];
                ...
                Buffer.BlockCopy(
                    clientEphemeralPublicKey, 0,
                    plaintext, 2 + keyBytes.Length,
                    PublicKeyLen);
        ...
        public static string Open(
            ...
            byte[] recipientPublicKey,
            byte[] sealedBox,
            out byte[] clientEphemeralPublicKey)
        {
            ...
            try
            {
                ...
                // [api_key_len:2 LE][api_key:N][client_ephemeral_pub:32] — reject
                // any length/frame mismatch.  The commitment is exact-width, not a
                // tail: a frame that may carry any number of trailing bytes is a
                // frame in which an otherwise identical envelope can be varied.
                if (plaintext.Length < 2) return null;
                int declared = plaintext[0] | (plaintext[1] << 8);
                if (declared != plaintext.Length - 2 - PublicKeyLen) return null;

                var commitment = new byte[PublicKeyLen];
                Buffer.BlockCopy(plaintext, 2 + declared, commitment, 0, PublicKeyLen);
                clientEphemeralPublicKey = commitment;
