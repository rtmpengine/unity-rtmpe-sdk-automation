...
namespace RTMPE.Crypto
{
    ...
    public static class ServerKeyPinning
    {
        ...
        /// reject the swap with a precise
        ...
        /// <summary>
        /// True when <paramref name="publicKey"/> is a well-formed X25519
        /// public-key encoding.
        ///
        /// <para>An X25519 public key is a u-coordinate reduced modulo
        /// 2²⁵⁵−19, so bit 255 — the top bit of the last byte — is always
        /// clear.  An Ed25519 public key stores the x-coordinate sign in that
        /// same bit and sets it for roughly half of all keys.  A set bit is
        /// therefore proof the value is not an X25519 public key, and by far
        /// the likeliest cause is the gateway's Ed25519 identity key pasted
        /// into the sealed-box field.</para>
        ///
        /// <para>The converse does not hold, and the check is not a
        /// substitute for one: an Ed25519 key whose sign bit happens to be
        /// clear is indistinguishable from a u-coordinate by encoding alone.
        /// Where a pin is configured, <see cref="ApiKeySealKeyMatchesPinnedKey"/>
        /// resolves the same mistake exactly, because it has a second copy of
        /// the identity key to compare against.  This predicate covers the
        /// case where it does not.</para>
        /// </summary>
        /// <param name="publicKey">Decoded 32-byte key material, or <see langword="null"/>.</param>
        ...
        /// or a pin already existed) or if the store refused the write, which
        /// the two are told apart by: <paramref name="failure"/> carries the
        /// store's exception in the second case and is <see langword="null"/>
        /// in the first.
        /// </returns>
        /// <param name="failure">
        /// The store's exception when the write was attempted and did not
        /// land, otherwise <see langword="null"/>.  A caller that ignores it
        /// silently downgrades the endpoint to a fresh capture on next
        /// connect.
        /// </param>
