...
namespace RTMPE.Crypto
{
    ...
    {
        /// <summary>
        /// First-time handshake.  The Round-1 payload is the transmitted
        /// <c>HandshakeInit</c> ciphertext.
        /// </summary>
        Init = 0,

        /// <summary>
        /// Reconnect handshake.  The Round-1 payload is the transmitted
        /// <c>ReconnectInit</c> payload, which carries the single-use
        /// reconnect token and therefore makes the transcript unique to this
        /// attempt.
        /// </summary>
        Reconnect = 1,
    }
    ...
    public sealed class HandshakeHandler : IDisposable
    {
       ...
       // These MUST match `modules/gateway/src/crypto/server_auth.rs` exactly,
        ...
        /// <summary>
        /// The exact size of a <c>Challenge</c> payload:
        /// [ephemeral:32][static:32][sig:64].
        /// </summary>
        /// <remarks>
        /// Public because the receive path has to know it before it decides what
        /// a frame costs. Framing is settled by this number alone — it is
        /// pre-AEAD, carries no secret and admits no side channel — so a frame
        /// that cannot possibly reach the verification below must not be charged
        /// for one.
        /// </remarks>
       ...
       /// If an optional pinned server public key is provided, it is also
        ...
        /// The exact payload bytes this client transmitted in Round 1 — the
        /// sealed API-key envelope for <see cref="HandshakeFlow.Init"/>, the
        /// <c>ReconnectInit</c> payload for <see cref="HandshakeFlow.Reconnect"/>.
        /// SHA-256 over these bytes is bound into the transcript, which is what
        /// ties a Challenge to one Round-1 packet and no other.  Required for
        /// both flows.
        /// </param>
        /// <param name="flow">
        /// Explicit flow selector.  It no longer decides which pre-image is
        /// built — both flows hash their Round-1 payload — but it still names
        /// the transcript the gateway signed, so a Challenge produced for one
        /// flow cannot be presented as the other.
        /// </param>
