...
//
// Parses physics-state payloads.
//
// Uplink frames are built by PhysicsPacketBuilder and sent as
// PacketType.PhysicsSync (0x45).  Nothing sends rigidbody state downward: the
// gateway has no Sync Service consumer for it and drops the frame, so the
// dispatch in NetworkManager.HandleStateSyncPacket that reaches these
// predicates off a StateSync (0x40) payload has no producer to fire for.  It
// is left in place, and a rigidbody downlink — when one exists — belongs on
// 0x45 with a receive handler of its own rather than on a byte peeked out of
// a payload that means something else under the transform reading.
//
// ── Disambiguation logic ──────────────────────────────────────────────────────
//
//  IsPhysics3D(payload) → true iff bit 0x40 set AND bit 0x80 clear.
//  IsPhysics2D(payload) → true iff bit 0x80 set.
//
//  The two predicates are mutually exclusive, so dispatch order does not
//  matter.  A frame carrying BOTH markers is one no builder writes: it selects
//  the 2-D predicate, and TryParsePhysicsState2D then refuses it, because
//  masking off the 2-D marker leaves the 3-D bit standing outside
//  DataFieldMask and the unknown-bit gate rejects it.  The gateway refuses the
//  same shape one layer earlier rather than counting it as a frame the builder
//  could have produced.
//
// ── Error handling ────────────────────────────────────────────────────────────
//
...
// function PhysicsPacketBuilder asks before it writes.  A NaN/Inf written into a
// Rigidbody silently destabilises PhysX, so it is refused at parse time.
    ...
    public static class PhysicsPacketParser
    {
        ...
        public static bool IsPhysics3D(byte[] payload)
        {
            if (payload == null || payload.Length < PhysicsPacketBuilder.PayloadMinSize)
                return false;
            byte mask = payload[PhysicsPacketBuilder.ChangedMaskOffset];
        ...
        public static bool IsPhysics2D(byte[] payload)
        {
            if (payload == null || payload.Length < PhysicsPacketBuilder.PayloadMinSize)
                return false;
            return (payload[PhysicsPacketBuilder.ChangedMaskOffset] & PhysicsPacketBuilder.TypeMarker2D) != 0;
        }
