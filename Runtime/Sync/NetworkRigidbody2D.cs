...
namespace RTMPE.Sync
{
    ...
    /// </para>
    /// <para>
    /// ⚠️ <b>Not driven end to end yet</b>, on the same terms as
    /// <see cref="NetworkRigidbody"/>: these frames are
    /// <c>PacketType.PhysicsSync</c> (0x45), and the gateway validates one and
    /// then drops it because no Sync Service consumer ingests rigidbody state.
    /// A remote replica is not driven by this component today — use
    /// <see cref="NetworkTransform"/> for anything a peer must actually see.
    /// </para>
    /// </summary>
    ...
    public class NetworkRigidbody2D : NetworkBehaviour
    {
        ...
        [Tooltip("Rotation error (degrees) above which the owner snaps to the server-confirmed " +
                 "angle.  Rotation diverges on a different scale from position, so it carries its " +
                 "own threshold rather than reusing the world-unit one above.")]
        ...
        /// <summary>
        /// Give the body back its authored type when the object leaves the
        /// network, so an instance returned to a pool is not handed back frozen.
        /// </summary>
