...
namespace RTMPE.Rooms
{
    ...
    public enum PropertyType
    {
        ...
        /// <summary>
        /// The deletion sentinel: "remove this key", not "this key holds
        /// nothing".  It is the empty type tag on the wire
        /// (<c>{"type":"","value":null}</c>) — the shape the Go server both
        /// accepts on a write and re-broadcasts to every other client
        /// (<c>entities.ApplyRoomProperties</c> deletes a key whose
        /// <c>PropertyValue.Type</c> is empty).
        /// </summary>
        /// <remarks>
        /// A value of this kind is a <b>message</b>, never a stored value: it
        /// appears in an update a caller hands to
        /// <see cref="RoomManager.SetRoomProperties"/> and in a broadcast
        /// delta, and <see cref="RoomInfo.MergeProperties"/> consumes it by
        /// removing the key.  It is therefore never observed in
        /// <see cref="RoomInfo.Properties"/> or
        /// <see cref="PlayerInfo.Properties"/>, and code reading a snapshot
        /// does not have to handle it.
        ///
        /// Declared LAST on purpose.  <c>default(PropertyValue)</c> has
        /// <c>Type == 0</c>, and moving this member to zero would silently
        /// reinterpret every default-constructed value as a deletion request.
        /// </remarks>
        Deleted,
    }
   ...
   /// Construct via one of the static factory methods (<see cref="OfInt"/>,
    ...
    public readonly struct PropertyValue : IEquatable<PropertyValue>
    {
        ...
        /// <summary>
        /// The deletion sentinel — pass it as the value of a key to remove
        /// that key from the room's or player's property map.  See
        /// <see cref="PropertyType.Deleted"/> for what the server does with it
        /// and why it never appears in a snapshot.
        /// </summary>
        public static PropertyValue Deletion()
            => new PropertyValue(PropertyType.Deleted);

        /// <summary>
        /// <see langword="true"/> when this value is the deletion sentinel
