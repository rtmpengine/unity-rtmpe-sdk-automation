...
//
// Outbound JoinRoom reliability for RoomManager.  On raw UDP with the ARQ
// capability negotiated — the stock configuration — the transport recovers a
// JoinRoom request whose datagram was lost, acknowledging one on receipt before
// the request is routed.  ⚠️ That is a configuration, not a guarantee: it needs
// `NetworkSettings.EmitArqSequence` on AND `CapabilityFlags.ArqAck` negotiated,
// and the gateway suppresses the cap on KCP and WebSocket by design.  There
// this ladder is the only recovery a lost request has.
//
// What no configuration recovers is a request that arrived and whose *reply*
// never came back: it is indistinguishable from one that never arrived, and no
// layer below this one can tell the difference or do anything about it.  Left
// unrecovered it wedges the client in a "joining…" state that never resolves.
// This primitive parks the exact request bytes and re-emits them on an
// exponential-backoff schedule until the response disarms it or the attempt
// budget is spent.  It is the request-side complement to the gateway's
// idempotent JoinRoom recovery: a resend that reaches an already-seated session
// is recovered server-side rather than rejected, so retransmission is safe.
//
// ⚠️ The caller supplies the resend, and it owes this ladder a single
// transmission rather than a fresh transport-level ladder per rung: nested,
// the two multiply, and the server answers every delivered copy by replaying
// the room's whole catch-up stream to the joining player.
//
