...
namespace RTMPE.Rooms
{
    ...
    public static class PropertyJson
    {
       ...
       // These MUST match `entities.PropertyType*` strings in the Go server.
        ...
        // The deletion sentinel is the EMPTY type tag, not a word.  The Go
        // server writes `{"type":"","value":null}` for a removed key
        // (entities.ApplyRoomProperties: `if v.Type == "" { delete(...) }`),
        // and a decoder with no arm for it throws — which discards the whole
        // broadcast, not just the deletion, because one payload carries every
        // key of one write.
        internal const string TagDeleted = "";
        ...
        /// </summary>
        /// <remarks>
        /// Both property packets are sent reliably, so a lost acknowledgement
        /// puts the same bytes on the wire again.  The server used to read the
        /// room from the session at the moment the copy arrived, which is a
        /// different room from the one the caller wrote for as soon as that
        /// caller has moved on.  The member below is what pins it, so a payload
        /// without one is refused here rather than encoded and sent.
        /// </remarks>
        private static void RequireRoomId(string roomId)
        {
            if (string.IsNullOrEmpty(roomId))
                throw new ArgumentException(
                    "roomId must not be null or empty — a property write names the room it is for.",
                    nameof(roomId));
        }
        ...
        /// </summary>
        /// <param name="roomId">The room this write is for.  Carried on the
        /// wire as <c>room_id</c> and compared by the gateway against the room
        /// the session occupies when the packet is delivered; a write that
        /// arrives after the sender has left that room is refused rather than
        /// applied to whichever room it is in by then.</param>
        /// <param name="version">The version the write expects to reach.</param>
        /// <param name="props">The properties to set.</param>
        public static string EncodeRoomPayload(
            string roomId, int version, IReadOnlyDictionary<string, PropertyValue> props)
        {
        ...
        /// <see cref="EncodeRoomPayload"/>.  ⚠️ It matters more here: a player's
        /// property version restarts at zero in every room, so the version
        /// check that would refuse a stale room-level write accepts a stale
        /// player-level one.</param>
        /// <param name="playerId">The player whose properties are written.</param>
        /// <param name="version">The version the write expects to reach.</param>
        /// <param name="props">The properties to set.</param>
        public static string EncodePlayerPayload(
            string roomId, string playerId, int version, IReadOnlyDictionary<string, PropertyValue> props)
        {
            RequireRoomId(roomId);
        ...
        private static string TagFor(PropertyType t)
        {
            switch (t)
            {
                ...
                case PropertyType.Deleted: return TagDeleted;
        ...
        private static void AppendValue(StringBuilder sb, PropertyValue v)
        {
            switch (v.Type)
            {
                ...
                case PropertyType.Deleted:
                    // The sentinel the server reads: an empty type tag and a
                    // null value.  `null` is the only value it may carry —
                    // the server ignores it, but a non-JSON token here would
                    // make the whole update undecodable on the far side.
                    sb.Append("null");
                    break;
       ...
       // Minimal recursive-descent parser — sufficient for the property
        ...
        private static PropertyValue DecodeByType(string type, string raw)
        {
            ...
            {
                case TagDeleted:
                    // "Remove this key", not a value.  MergeProperties consumes
                    // it; it never reaches a snapshot the caller reads.
                    return PropertyValue.Deletion();
                case TagInt:
                    if (!int.TryParse(
                            raw,
