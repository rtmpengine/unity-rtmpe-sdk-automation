...
namespace RTMPE.Rooms
{
    ...
    public sealed class PendingCreateTable<T>
    {
        ...
        /// </summary>
        /// <remarks>
        /// FIFO fallback exists for a gateway that does not return the request
        /// id — see the wire-protocol coordination note in RoomManager.  Against
        /// one that does, every match is id-based and FIFO is defence in depth.
        ///
        /// ⛔ A `false` return is load-bearing rather than a degraded match: it
        /// says this response answers no request the caller still holds, and
        /// RoomManager declines to apply it on that basis.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// 🔑 Eager, and it must stay eager.  The removals are complete before
        /// the first tuple is visible to the caller, so what the caller does
        /// with them cannot reach a half-swept table.
        ///
        /// ⛔ Written as a C# iterator this was a live fault, not a style
        /// question: the documented purpose of the return value is to be
        /// surfaced to observers, so the caller runs APPLICATION code between
        /// two of these removals — and an application handling "CreateRoom
        /// timed out" may perfectly well disconnect (which reaches
        /// <see cref="Clear"/>) or retry (which reaches this method again).
        /// Either empties or re-sweeps the dictionary that the suspended
        /// enumeration is still indexing by key, and the next
        /// <c>MoveNext</c> threw <see cref="KeyNotFoundException"/> out of
        /// whatever was driving it.
        ///
        /// The empty result is shared rather than allocated, so a sweep that
        /// finds nothing — every sweep, in ordinary play — costs nothing to
        /// call.
        /// </remarks>
