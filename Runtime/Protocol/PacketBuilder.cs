...
namespace RTMPE.Protocol
{
    ...
    public sealed class PacketBuilder
    {
        ...
        /// </summary>
        /// <remarks>
        /// <paramref name="sealedApiKey"/> has no default on purpose.  A
        /// gateway refuses a <c>HandshakeInit</c> whose flag is clear, so a
        /// convenience overload would hand every caller that omitted the
        /// argument a packet that cannot be answered.
        /// </remarks>
        ...
        /// which the gateway requires before it will read the payload.  Passing
        /// <see langword="false"/> builds a packet the gateway refuses; it exists
        /// so the protocol test can pin what the bit means.</param>
        ...
        /// <see cref="RTMPE.Core.Protocol.CapabilityFlags.None"/>.</item>
        /// <item><b>New gateway + new SDK:</b> the advertised cap sets
        /// intersect, and the gateway pins the wire format to
        /// `min(client_pref, gateway_max)`.  ⚠️ The two are not symmetric:
        /// the gateway returns its cap bitmask in the <c>SessionAck</c> tail,
        /// but it returns no wire-format byte, so the preference below is a
        /// request the SDK never hears the answer to.  The gateway's ceiling
        /// is <see cref="WireFormatVersion.V2"/> today — a V4 preference is
        /// clamped — and no code path on either side branches on the pinned
        /// value, so the disagreement is a record, not a behaviour.</item>
        /// </list>
        /// </summary>
        ...
        /// <summary>
        /// Byte offset of the flags field inside the 13-byte fixed header.
        /// </summary>
        /// <remarks>
        /// 🔑 Declared because the flags byte is now READ as well as written:
        /// the send path derives a packet's reliability from the bit the
        /// builder set, rather than taking it as a second argument that can
        /// disagree with the packet.  Two hand-written `4`s either side of that
        /// derivation would be the same drift one layer down, so the offset is
        /// written once and both ends use it.
        /// </remarks>
        ...
        /// <summary>
        /// The largest payload the gateway is permitted to put on the wire,
        /// mirroring <c>MAX_PAYLOAD_SIZE</c> in
        /// <c>modules/gateway/src/packet/types.rs</c>.
        /// </summary>
        /// <remarks>
        /// This bounds what the client must be able to RECEIVE, which is a
        /// different question from what it may send: outbound frames are held to
        /// <see cref="MaxApplicationPayloadBytes"/> so they survive every link,
        /// while inbound ones are whatever the server built. A full lobby room
        /// list already exceeds a typical scratch buffer on its own, and a
        /// datagram larger than the buffer it is read into is truncated by the
        /// socket, fails its authentication tag and is discarded before any
        /// parser runs.
        /// <para>Held equal to the gateway constant by
        /// <c>scripts/check-reply-capacity-contract.sh</c>.</para>
        /// </remarks>
        ...
        /// <summary>
        /// The smallest receive buffer that can hold any datagram able to
        /// arrive: the largest a UDP path will carry.
        /// </summary>
        /// <remarks>
        /// A UDP length field is sixteen bits and counts its own eight-byte
        /// header, so 65 527 bytes of payload is the ceiling — measured as
        /// 65 507 over IPv4 (twenty further bytes of IP header) and 65 527 over
        /// IPv6, with anything larger refused outright by <c>sendto</c>.
        /// <para>
        /// It is deliberately NOT derived from what the gateway can construct.
        /// <see cref="GatewayMaxPayloadBytes"/> bounds the bytes handed to the
        /// gateway's AEAD seal, and the datagram carries the header and the
        /// seal's own additions on top, so the gateway can build frames of
        /// 65 540 bytes — thirty-three past this ceiling, and unsendable. A
        /// buffer sized for those covers datagrams that cannot exist while
        /// pushing every rental into the next allocator bucket; the gap belongs
        /// to the gateway, whose producers budget against its own
        /// <c>MAX_SEALED_PAYLOAD_SIZE</c>.
        /// </para>
        /// A datagram larger than the buffer it is read into is lost before
        /// any parser runs — truncated by the socket on POSIX and then failing
        /// its authentication tag, refused outright with <c>WSAEMSGSIZE</c> on
        /// Windows — and neither outcome reports anything to the caller. The
        /// floor beneath <c>NetworkSettings.networkThreadBufferBytes</c> is
        /// therefore this value and not a smaller one chosen for expected
        /// traffic.
        /// </remarks>
        ...
        /// <summary>
        /// Largest application payload that still fits inside one
        /// <see cref="DefaultDatagramSizeMirror"/>-byte UDP datagram once the
        /// 13-byte RTMPE header, the sub-header region and the AEAD overhead
        /// are accounted for.
