...
//
// Total: 42 + owner_len bytes, or 43 + owner_len with the authority
// declaration.  The tail ahead of it is fixed-length, so the trailing byte is
// unambiguous: a gateway reading 42 + owner_len sees no declaration and
// resolves the object owner-only.
//
...
namespace RTMPE.Core
{
    ...
    public static class SpawnPacketBuilder
    {
            ...
            byte authority = SpawnAuthorityFlags.OwnerOnly)
        {
            ...
            // 4 + 8 + 2 + N + 7*4 + 1 = 43 + N
            int size = 4 + 8 + 2 + ownerBytes.Length + 28 + 1;
