...
//
// Two sources, one handler: the console channel above carries a severity that
// has already been adjusted for the host app's crash reporters, and the SDK's
// own routine faults arrive there indistinguishable from ordinary traces. Those
// faults are taken from RtmpeLog.Downgraded instead, at the severity the
// condition holds. The two are disjoint by construction — RtmpeLog raises the
// event only where it lowers the console severity, and a lowered entry is a
// LogType.Log, which ShouldCapture declines — so the handlers below are shared
// between the sources rather than duplicated, and every capture bound they
// apply covers both.
...
namespace RTMPE.Core.Diagnostics
{
    ...
    internal sealed class DiagnosticsUplink
    {
        ...
        public void Stop()
        {
            ...
            // are not recaptured.
            DetachSessionCapture();
            FlushRemaining();
        }

        // Both capture sources move together, once per handler. A handler
        // attached to one and not the other is a half-deaf uplink, or a
        // subscriber that outlives the session it belonged to; neither shows
        // from outside the type, because a stopped uplink drains nothing and a
        // detach it failed to perform therefore produces no entry to observe.
        // Writing the pair in one place is what makes the two cases the same
        // edit rather than two that have to be remembered together.
        private void AttachPreSessionCapture()
        {
            Application.logMessageReceivedThreaded += OnPreSessionLogThreaded;
            RtmpeLog.Downgraded += OnPreSessionLogThreaded;
        }

        private void DetachPreSessionCapture()
        {
            Application.logMessageReceivedThreaded -= OnPreSessionLogThreaded;
            RtmpeLog.Downgraded -= OnPreSessionLogThreaded;
        }

        private void AttachSessionCapture()
        {
            Application.logMessageReceivedThreaded += OnLogThreaded;
            RtmpeLog.Downgraded += OnLogThreaded;
        }

        private void DetachSessionCapture()
        {
            Application.logMessageReceivedThreaded -= OnLogThreaded;
            RtmpeLog.Downgraded -= OnLogThreaded;
        }
