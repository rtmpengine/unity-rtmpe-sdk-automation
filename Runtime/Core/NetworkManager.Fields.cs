...
namespace RTMPE.Core
{
    public sealed partial class NetworkManager
    {
       ...
       // Holds the exact bytes the client emitted as the HandshakeInit
        ...
        /// <summary>
        /// Time (seconds) elapsed since the most recent simulation tick boundary,
        /// i.e. the tick driver's leftover accumulator after this frame advanced
        /// <see cref="LocalTick"/>.  Always in <c>[0, FixedTickInterval)</c>
        /// except on the frame a long hitch is surrendered, where it is zero.
        ///
        /// <para>An owner samples its transform at the frame instant but labels
        /// the sample with the current tick; this is the distance between the
        /// two, and the quantity an owner needs to report the pose it held on the
        /// boundary rather than the one it holds mid-tick.  The tick loop runs
        /// from <see cref="NetworkManager"/>'s Update, pinned ahead of ordinary
        /// components by its execution order, so a component reading this in its
        /// own Update observes the current frame's residual.</para>
        /// </summary>
       ...
       // The counters are read by the Editor-only Network Debugger window
        ...
        /// against the new key.
        /// <para>Propagates whatever the configured store raises.  The default
        /// store absorbs a storage failure and logs it, so this throws only for
        /// an application-supplied store — and a clear that did not happen is
        /// worth hearing about, because the old pin is still in force.</para>
        /// </summary>
        /// <exception cref="System.IO.IOException">
        /// The store could not rewrite its backing file.
        /// </exception>
       ...
       // Snapshot accessors return the current counter value.  Subtract two
        ...
        /// <summary>
        /// Transport faults absorbed as costing one datagram rather than the
        /// session — an oversize frame, an ICMP port-unreachable, a transient
        /// routing failure.  Each one is a packet that did not cross the wire
        /// while the connection stayed up, so sustained growth is packet loss
        /// with no other symptom: the alternative to counting these is ending
        /// the session, which is why nothing else reports them.  Reads
        /// <c>0</c> with no live thread.
        /// </summary>
        ...
        /// <summary>
        /// The tick every outgoing <c>NetworkVariable</c> update is stamped
        /// with, and the counter a receiver orders those updates by.
        /// </summary>
        /// <remarks>
        /// Driven by a real-time cadence, unlike <see cref="LocalTick"/>, which
        /// indexes simulation steps and stops with the game's own clock.  The
        /// two are unrelated numbers and must not be compared with one another;
        /// nothing on the wire correlates them, because a receiver uses this one
        /// only to order a single variable's updates against each other.
        ///
        /// ⚠️ A counter of flushes, not a timebase.  It advances only while in a
        /// room, at most <c>MaxTicksPerFrame</c> per frame, and the surplus after
        /// a long hitch is discarded — so differencing it does NOT measure
        /// elapsed time across a stall or a pause.
        /// </remarks>
