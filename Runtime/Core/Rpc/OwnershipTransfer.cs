...
//   exist: initial-assignment, self-initiated, host-authorised, and
//   server-relayed — any wrongly-accepted grant is an
//   ownership-flip
...
namespace RTMPE.Core.Rpc
{
    ...
    {
        // The budget for the INBOUND authorisation refusal in HandleRpc, which a
        // remote peer drives one line per RPC (`RPC-RD-05`).
        //
        // ⛔ `HandleResponse`'s disagreement line below is deliberately NOT on
        // this budget, and `RPC-RD-05` counted it as a second site by mistake.
        // It answers a request THIS client issued, and
        // `OwnershipResponseAgreementTests.AnyDisagreement_IsReported` requires
        // every disagreement between `success` and `errorCode` to be reported —
        // a property a rate limit silently breaks.  🚨 Gating it made three
        // `[Theory]` cases share one static budget inside a single second, and
        // the second and third were suppressed: the same hazard as a gate that
        // is static per closed generic, and the reason a shared budget is only
        // ever right for two sites reporting ONE condition.
        private static long s_lastUnauthorisedTransferWarnTicks;
        ...
        /// <param name="localPlayerStringId">SDK's <c>_localPlayerStringId</c>. Used for the self-initiated path.</param>
            ...
            RoomManager roomManager)
        {
            ...
            {
                // One line per inbound ownership RPC a remote peer sends
                // (`RPC-RD-05`).  The refusal above is NOT gated — only the line
                // reporting it is.
                if (WarnGate.ShouldEmit(ref s_lastUnauthorisedTransferWarnTicks))
                    Debug.LogWarning(
            ...
            RoomManager roomManager)
        {
            ...
            // Empty new-owner is the wire encoding for "release ownership", and
            // no release is authorised — not the owner's own, not the master
            // client's.  The gateway refuses an empty transferee at ingress
            // (audit A6-3b), and its per-object fail-open argument is stated as
            // depending on that refusal: an alive-but-unowned object is
            // mutable by any room member, not by none.  So a release cannot
            // arrive from an honest gateway, and admitting one here would
            // manufacture locally the state the protocol refuses to carry.
            //
            // Refused here as well as at the apply step because a predicate
            // that returns true for a grant the next layer drops is a rule the
            // tree states twice and answers two ways — the shape that let this
            // branch stay live while nothing could reach it.
        ...
        public static void HandleResponse(RpcResponse response, SpawnManager spawnManager)
        {
            ...
            // A success has to be agreed by BOTH fields. They are independent on
            // the wire and this frame is relayed from a peer, so a reading that
            // believes one of them is the reading a sender picks: a response
            // carrying success=1 beside a non-OK code would be a rejection this
