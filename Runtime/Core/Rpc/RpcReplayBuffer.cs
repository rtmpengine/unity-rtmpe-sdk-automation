...
namespace RTMPE.Core.Rpc
{
    internal sealed class RpcReplayBuffer
    {
        ...
        // Once-per-room-entry admission. 0 = no replay accepted since the
        // last room entry, 1 = one has been. Distinct from the barrier above,
        // which is lowered the moment both queues empty and therefore says
        // nothing about a frame that arrives after the first has drained.
        private int _replayAdmitted;
        ...
        /// <summary>
        /// Admit a newly-arrived RpcBufferReplay frame and raise the ordering
        /// barrier for it. Returns <see langword="true"/> for the first frame
        /// of a room entry and <see langword="false"/> for every later one.
        ///
        /// <para>Admission is deliberately not the barrier. The barrier is
        /// lowered by the final fully-drained <see cref="DrainBounded"/> pump,
        /// so on its own it rejects only a frame that overlaps a drain still
        /// in progress; a second frame arriving after the first has finished
        /// would pass and dispatch every buffered event again — for an
        /// <c>AllBuffered</c> RPC that means the score or inventory grant it
        /// carries is applied twice. AEAD replay protection does not cover
        /// this: it rejects a retransmitted datagram, not a second frame
        /// sealed under a fresh nonce.</para>
        ///
        /// <para>Re-armed by <see cref="ResetForRoomEntry"/> on each room
        /// entry and by <see cref="Clear"/> at the session boundary, so a
        /// room switch and a reconnect both receive their own catch-up.</para>
        /// </summary>
        public bool TryEnterDrain()
        {
            if (Interlocked.CompareExchange(ref _replayAdmitted, 1, 0) != 0)
                return false;

            Interlocked.Exchange(ref _replayInProgress, 1);
            return true;
        }

        /// <summary>
        /// Re-arm the once-per-entry admission for a newly-entered room.
        /// Must run before any frame staged ahead of the join is released,
        /// or the previous room's admission would swallow this room's
        /// catch-up.
        /// </summary>
        public void ResetForRoomEntry()
            => Interlocked.Exchange(ref _replayAdmitted, 0);
