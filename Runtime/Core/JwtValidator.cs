...
namespace RTMPE.Core
{
    internal sealed class JwtValidator
    {
        ...
        //
        // CS0649 is suppressed across both shapes because the deserialiser
        // assigns these fields reflectively: no assignment appears in source
        // by design, and the warning would otherwise fire once per claim on
        // every build.  The suppression is scoped to the two declarations so
        // a genuinely unassigned field anywhere else still reports.
        [Serializable]
        private sealed class JwtHeaderDto
        {
            // ReSharper disable InconsistentNaming — JSON contract.
#pragma warning disable IDE1006, CS0649
            public string alg;
            public string typ;
            public string kid;
