...
namespace RTMPE.Core
{
    public sealed partial class NetworkManager
    {
        ...
        // The pinning verdict does not need a frame, and until CORE-RD-12 it was
        // only ever taken on one — inside OnChallenge, where a bare 13-byte 0x06
        // could trigger a full teardown and destroy the reconnect token with it.
        // Every input is local: the mode, the configured pin, the pin store and
        // the endpoint.  So both Round-1 senders decide it before anything
        // reaches the wire — beside the configuration mistakes SendHandshakeInit
        // already refuses — and the Challenge handler is left with nothing to
        // act on.  ⛔ Both, not one: a reconnect draws the same Challenge through
        // the same handler, so covering only the fresh connect would leave the
        // reconnect's diagnosis to a ten-second timeout.
        //
        // ⛔ Resolved exactly once per attempt and cached, not recomputed per
        // frame.  `PreparePin` itself is stateless, but the store it reads is
        // not cheap and not side-effect-free — see the field's declaration in
        // NetworkManager.HandshakeHandlers.cs for what one call actually costs.
        // Caching also removes a disagreement that would otherwise exist: the
        // verdict is NOT deterministic (an unreadable store refuses, and reads
        // again a moment later), so resolving twice could answer twice.
        private bool PinConfigurationRefusesToConnect(out string reason)
        {
            reason = null;
            byte[] configuredPin;
            ...
            catch (Exception ex)
            {
                ...
                reason = "pinnedServerPublicKeyHex is not a valid Ed25519 public key: " + ex.Message;
                return true;
            }

            var mode = _settings != null ? _settings.EffectivePinningMode : ServerPinningMode.Strict;
            _attemptPinResolution = ServerKeyPinning.PreparePin(
                mode,
                configuredPin,
                PinStore,
            ...
            _attemptPinResolved = true;

            if (_attemptPinResolution.Decision != PinDecision.RefuseStrictNoPin) return false;

            _attemptPinResolved = false;
            reason =
                "Server not pinned — refusing handshake. No pin was configured, none was " +
                "pre-provisioned, or the pin store could not be read.";
            return true;
        }

        private void SendHandshakeInit(string apiKey)
        {
            // Refused before a byte reaches the wire: the verdict is local, so
            // spending the connect-timeout watchdog on it — or waiting for a
            // frame to trigger it — helps nobody.
            if (PinConfigurationRefusesToConnect(out string pinRefusal))
            {
                Debug.LogError(
                    $"[RTMPE] SendHandshakeInit: {pinRefusal}  Either set " +
                    "NetworkSettings.pinnedServerPublicKeyHex (Strict), pre-provision a pin via " +
                    "IServerKeyPinStore (TrustOnFirstUse + requireFirstUseProvisioned), or relax " +
                    "requireFirstUseProvisioned if first-flight TOFU capture is acceptable for " +
                    "this deployment.  Aborting connection.");
                FailHandshake(pinRefusal, DisconnectReason.ProtocolError);
                return;
            }

            // The API key is sealed to the gateway's static X25519 public key
            // (anonymous sealed box), so this build carries no secret of the
            // gateway's — which matters because a shipped game can be unpacked.
            // It is the only envelope the gateway accepts: everything past this
            // block is the refusal for a build that has no key to seal to.
            ...
            {
                // The sealed-box field takes the gateway's X25519 agreement key
                // and the pin takes its Ed25519 signing identity.  They are
                // different credentials that never coincide, so an exact match
                // means the identity key was pasted into the sealed-box field:
                // the box goes out sealed to a point the gateway holds no scalar
                // for, is discarded without a reply, and the only symptom is the
                // connect-timeout watchdog ten seconds later with nothing to
                // attribute it to.  The editor reports the same state on the
                // asset, which a settings object built in code or fetched from
                // remote config never passes through.
                if (_settings != null
                 && ServerKeyPinning.ApiKeySealKeyMatchesPinnedKey(
                        _settings.apiKeySealServerPublicKeyHex,
                        _settings.pinnedServerPublicKeyHex))
                {
                    Debug.LogError(
                        "[RTMPE] SendHandshakeInit: apiKeySealServerPublicKeyHex equals " +
                        "pinnedServerPublicKeyHex — the sealed-box field takes the gateway's " +
                        "X25519 key agreement key, the pin takes its Ed25519 identity key. " +
                        "Copy the X25519 key from the gateway-config endpoint into " +
                        "apiKeySealServerPublicKeyHex. Aborting connection.");
                    FailHandshake(
                        "apiKeySealServerPublicKeyHex equals pinnedServerPublicKeyHex — the " +
                        "Ed25519 identity key was placed in the sealed-box field.",
                        DisconnectReason.ProtocolError);
                    return;
                }

                // Independent of the pin, and so still available when none is
                // configured: an X25519 u-coordinate is reduced modulo 2^255-19
                // and never carries bit 255, while an Ed25519 key stores its
                // x-sign there.  A set bit proves the value is not a public key
                // this box can be sealed to.
                if (!ServerKeyPinning.IsWellFormedX25519PublicKey(sealRecipient))
                {
                    Debug.LogError(
                        "[RTMPE] SendHandshakeInit: apiKeySealServerPublicKeyHex is not a " +
                        "valid X25519 public key — bit 255 is set, which an X25519 " +
                        "u-coordinate never has and an Ed25519 public key often does. " +
                        "The gateway's Ed25519 identity key was most likely pasted here; it " +
                        "belongs in pinnedServerPublicKeyHex. Aborting connection.");
                    FailHandshake(
                        "apiKeySealServerPublicKeyHex is not a valid X25519 public key.",
                        DisconnectReason.ProtocolError);
                    return;
                ...
                try
                {
                    sealedPayload = SealedApiKeyCipher.Seal(
                        sealRecipient, apiKey, _handshakeHandler.ClientPublicKey);
                }
                catch (Exception ex)
                {
                    Debug.LogError(
                ...
                // Transcript channel binding hashes exactly these payload
                // bytes; the gateway hashes the same sealed box off the wire, so
                // the two transcripts agree only if nothing altered the envelope
                // in flight.
                _lastHandshakeInitCiphertext = sealedPayload;
                _lastRoundOnePayload = sealedPayload;
                ...
                // Witness for the timeout diagnostic: the init reached the wire,
                // so a later connect timeout resolves to the NoServerReply rung
                // (the gateway never answered) rather than the earlier "init not
                // dispatched" rung.
                _diagHandshakeInitSent = true;
                return;
            }

            // Past the sealed branch there is nothing left to attempt.  The
            // sealed box is the only envelope packet 0x05 carries, and a
            // gateway refuses one that does not declare it before reading a
            // byte of the payload — so emitting anything here would spend the
            // connect-timeout watchdog on a packet that was never going to be
            // answered.  Naming the misconfiguration now is the same fail-fast
            // contract the Strict-pinning refusal above uses.  Loopback is not
            // an exception: the plaintext development path existed because the
            // symmetric envelope was optional, and it is not optional to have
            // an envelope any more.
            Debug.LogError(
                "[RTMPE] SendHandshakeInit: apiKeySealServerPublicKeyHex is not set. The " +
                "gateway accepts only a HandshakeInit sealed to its static X25519 public " +
                "key — copy that key from the gateway-config endpoint into " +
                "apiKeySealServerPublicKeyHex in NetworkSettings. Aborting connection.");
            FailHandshake(
                "No API-key envelope configured — set apiKeySealServerPublicKeyHex.",
                DisconnectReason.ProtocolError);
        }
        ...
        /// reconnect token.  Payload is plaintext (the token and its proof
        /// ARE the authentication) and does NOT go through
        ...
        private void SendReconnectInit(string token)
        {
            ...
            // The same local verdict the fresh-connect path takes — but ⛔ the
            // teardown is NOT the same, and an earlier draft of this got it
            // wrong in a way that cost a credential.  One arm of this verdict is
            // TRANSIENT: `ServerKeyPinning` refuses a store it could not read,
            // in its own words *"a connect that succeeds once the store is
            // reachable again"*.  A file held open for a moment during an
            // auto-reconnect would therefore have wiped a reconnect token the
            // gateway had not spent, and `CanReconnect` would go false for good
            // — the exact harm this whole finding is about, moved from a frame
            // trigger to a configuration one.  So the attempt fails fast and the
            // token survives to the next one.
            if (PinConfigurationRefusesToConnect(out string pinRefusal))
            {
                Debug.LogError(
                    $"[RTMPE] SendReconnectInit: {pinRefusal}  Reconnection cannot proceed under " +
                    "the current pinning configuration; see the SendHandshakeInit guidance for " +
                    "the three ways to resolve it.  The reconnect token is kept: if the pin " +
                    "store was merely unreadable, a later attempt succeeds.");
                FailHandshake(pinRefusal, DisconnectReason.ProtocolError,
                              preserveReconnectToken: true);
                return;
            }
            ...
            try
            {
                if (hasProof)
                {
                    var proof = RTMPE.Protocol.PacketBuilder.ComputeReconnectProof(
                        token, _handshakeHandler.ClientPublicKey, _ipMigrationKey);
                    packet = _packetBuilder.BuildReconnectInit(
                        token, _handshakeHandler.ClientPublicKey, proof);
                }
                else
                {
                    packet = _packetBuilder.BuildReconnectInitWithoutProof(
                        token, _handshakeHandler.ClientPublicKey);
                }
            }
            ...
            // Bind this attempt's Challenge to the packet that opened it.  The
            // gateway hashes the payload, not the framed packet, so the slice
            // has to match what it will digest.
            _lastRoundOnePayload = PacketParser.ExtractPayload(packet);
        ...
        private IEnumerator ConnectionTimeoutRoutine()
        {
            ...
            // Read before the wait, so it names the attempt that armed this
            // watchdog rather than whatever is live when it expires.
            ...
            // Release the handle here, before anything below can reach
            // application code, rather than after the teardown as it once was.
            // From this line the routine only unwinds, so the handle is already
            // stale; and everything below raises events — OnConnectionFailed,
            // then OnDisconnected — into which an application is told to call
            // Connect().  That call installs the next attempt's watchdog in this
            // same field, and clearing the field afterwards discarded it: the
            // new attempt kept a watchdog nothing could stop, SessionAck's
            // StopCoroutine found null, and the timeout fired a few seconds
            // later against a connection that had succeeded.
            ...
            if (AttemptIsStillLive(epoch)
             && (_state == NetworkState.Connecting || _state == NetworkState.Reconnecting))
            {
                ...
                // A pre-session rejection recorded during this attempt is the
                // actionable account of why it failed, and the watchdog is where
                // it surfaces: the two handlers that receive one (Challenge,
                // HandshakeError) drop the frame rather than act on it, because
                // neither carries anything this client can authenticate.  Read
                // BEFORE the teardown below, which clears it.
                SafeRaise(OnConnectionFailed,
                    _pendingHandshakeRejection
                        ?? (wasReconnecting ? "Reconnect timeout." : "Connection timeout."),
                    nameof(OnConnectionFailed));

                // The handler above may have disconnected and reconnected, which
                // both succeed from there, and everything below would then be
                // spent on the attempt that call started — closing its socket,
                // stopping its handshake coroutine and reporting it timed out.
                if (!AttemptIsStillLive(epoch)) yield break;
                ...
                //
                // The witness is _diagChallengeValidated, not the arrival
                // witness beside it: the latter is set before validation on
                // purpose, so a forged Challenge would otherwise make this
                // client discard a reconnect token that is still good.  It is
                // also the tighter reading of the IP-migration clause above —
                // that write lives past validation too, so a Challenge that
                // arrived and was rejected never overwrote the proof material.
                //
                // ⛔ It is a TRADE, not a strict improvement, and the cost is
                // named here so it is not rediscovered as a defect.  The gateway
                // spends the token when it ACCEPTS the ReconnectInit, before it
                // answers — so a genuine Challenge corrupted in flight, or one
                // refused by a pin the operator has just rotated, now leaves
                // this witness false and the client keeps a token the gateway
                // has already consumed.  It then replays that token for the rest
                // of the bounded loop: the refusal comes back on an
                // unauthenticated frame, and a production gateway collapses every
                // error category onto the generic code, so no attempt can tell it
                // from a gateway that is simply unreachable and stand down early
                // without handing a spoofed datagram the same power.  The budget
                // is therefore the bound, and the loop drops the token when that
                // budget is spent — so the cost is one reconnect budget in a rare
                // case, against a credential an attacker could otherwise delete
                // with one spoofed datagram in the common one.
                bool keepReconnectToken = wasReconnecting && !_diagChallengeValidated;
        ...
        {
            // Central teardown for the two per-session timers, stopped here
            // rather than at each call site because this is the one chokepoint
            // every session-ending path funnels through.
            //
            // The uplink's capture hook is a PROCESS-STATIC Unity event
            // (Application.logMessageReceivedThreaded), so it must be
            // unsubscribed on every such path or the instance leaks and keeps
            // capturing for the life of the process.
            //
            // The heartbeat is the reason this rule is stated here instead of
            // being left to the call sites: it was, and one site forgot.  The
            // gateway-initiated OnServerDisconnect stopped the network thread
            // and cleared the session without stopping the liveness timer, so a
            // session the server had already ended kept ticking from Update —
            // sending keep-alives into a torn-down session and then, some
            // seconds later, firing OnHeartbeatTimeout against whatever
            // connection was live by then.  Applications are told to call
            // Connect() from OnDisconnected, so "whatever was live by then" is
            // routinely the attempt that disconnect started.
            //
            // Stop() is idempotent on both, so the SessionAck re-create and
            // Cleanup paths that stop them explicitly are unaffected; each
            // instance is re-created on the next SessionAck.
            ...
            // Both managers hold session-scoped deadlines, and both are normally
            // discarded with the session — which is exactly why they are cleared
            // here as well: a watch that survives its link either speaks about a
            // connection that can no longer answer, or is thrown away before it
            // ever does.
            _lobbyManager?.ClearState();
            // And the scene load the room was in the middle of.  Nothing else
            // reaches it on this path: a disconnect neither replaces the room
            // manager nor raises OnRoomLeft, and the MonoBehaviour survives, so
            // the frame loop would go on reading a deadline whose room can no
            // longer answer it.
            _sceneManager?.ClearState();
            ...
            // The advisory latch too, and directly rather than through the
            // ClearAll above: that call is null-conditional, so a teardown
            // arriving after the spawn manager is gone would leave the session's
            // surfaced ids behind — the same shape as the JWT and the verifier
            // hooks, which are released here for the same reason. ResetLatch is
            // idempotent, so the two paths cost nothing when both run.
            ...
            // The inbound flood budget is scaled to a room's fan-out at entry
            // and was restored only by an EXPLICIT leave — so a client that
            // dropped out of a room carried a room-sized budget into its next
            // connect, which has no room and no fan-out to justify it.  That
            // matters here beyond tidiness: the pre-validation region of the
            // handshake is admitted under this budget, so a 100-player room's
            // 15 150 pps became the ceiling on how fast a spoofed Challenge
            // could be delivered to the next attempt.  Restored on the same
            // chokepoint every other per-session fact is.
       ...
       /// <para>When session keys are present the following transformations are applied,
        ...
        /// </summary>
        // The two SEND-path budgets this method spends.  Separate, because one
        // reports a session approaching its end and the other a builder that is
        // producing packets the transport refuses; a run of either must not
        // decide whether the other is ever printed.
        private long _lastNonceNearExhaustionWarnTicks;
        private long _lastEncryptPayloadLenWarnTicks;
        ...
        private void EncryptAndSendInternal(byte[] packet, bool hasFixedArqSeq, uint fixedArqSeq)
        {
            ...
            // ⛔ Gated, because this condition STAYS true once it is true: the
            // margin is ~1 M packets, so an ungated line here wrote roughly a
            // million of them over the ~9.7 h at 30 Hz it is meant to give the
            // application warning of.  The advisory is the whole of what the
            // branch does, so a closed gate skips nothing else; the hard stop
            // above it is unconditional and ends the session.
            if (rawCounter >= OutboundNonceExhaustionThreshold - OutboundNonceNearExhaustionMargin
                && ShouldWarn(ref _lastNonceNearExhaustionWarnTicks))
                Debug.LogWarning(
            ...
            {
                // The refusal is outside the gate: a malformed packet is
                // dropped on every arrival, and only the line is bounded.  A
                // builder fault is deterministic, so it repeats at send rate.
                if (ShouldWarn(ref _lastEncryptPayloadLenWarnTicks))
                    Debug.LogError(
        ...
        private byte[] DecryptInboundPacket(byte[] data, int length)
        {
            ...
            if (window == null)
            {
                // 🚨 Found by the member-scoped rule while repairing
                // `RPC-RD-05`, which named the LZ4 site in this same method and
                // not this one.  The condition is a state fault, so once it
                // holds it holds for every subsequent AEAD frame — a console
                // line per datagram for as long as the session lasts.
                // ⛔ The REFUSAL is outside the gate: the frame is dropped
                // whether or not the line is written.
                if (ShouldWarn(ref _lastReplayWindowMissingWarnTicks))
                    Debug.LogWarning(
            ...
            if (wasCompressed && plaintext.Length > 4)
            {
                ...
                {
                    // One line per inbound COMPRESSED frame that will not
                    // decompress, and a sender that emits one emits them
                    // (`RPC-RD-05`).  Gated on the same terms as the AEAD
                    // failure and the replay-window drop two screens up, which
                    // this sits beside on the same path.
                    if (ShouldWarn(ref _lastLz4DecompressFailWarnTicks))
                        Debug.LogWarning(
