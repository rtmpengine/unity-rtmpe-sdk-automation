...
namespace RTMPE.Core
{
    public sealed partial class NetworkManager
    {
        ...
        /// <summary>
        /// Whether this component won the singleton race in <see cref="Awake"/>.
        /// </summary>
        /// <remarks>
        /// <para>
        /// 🔑 The teardown in <see cref="OnDestroy"/> is not this component's
        /// alone: it resets <c>EnhancedRpcVerifier</c>'s hooks and the gated
        /// logger's settings, both PROCESS-STATIC. A duplicate rejected here is
        /// destroyed immediately, its <c>OnDestroy</c> runs, and it used to run
        /// that teardown against the live manager's state — nulling
        /// <c>SelfSessionIdProvider</c>, which by its own documentation makes
        /// the default verifier reject every sender. Additively loading a scene
        /// whose prefab carries a <c>NetworkManager</c> is an ordinary thing to
        /// do, and it silently ended RPC dispatch for the session.
        /// </para>
        /// <para>
        /// ⚠️ A field rather than an <c>_instance == this</c> test at teardown.
        /// The two agree today, but the question being asked is "did this
        /// component ever own the process-wide state?", and only the component
        /// that assigned itself can answer it — including for a component
        /// destroyed before <c>Awake</c> ever ran, which owns nothing and must
        /// tear down nothing.
        /// </para>
        /// </remarks>
        private bool _ownsProcessState;
        ...
        private void Awake()
        {
            lock (_instLock)
            {
                ...
                {
                    // Promised by name in two places in getting-started.md —
                    // "duplicate-singleton warnings" and a troubleshooting entry
                    // headed `Duplicate NetworkManager instance` — and emitted
                    // nowhere until 2026-08-27. A silent Destroy leaves the
                    // reader looking for a message that was never written.
                    Debug.LogWarning(
                        "[RTMPE] Duplicate NetworkManager instance on GameObject " +
                        $"'{gameObject.name}' — destroying it and keeping the existing one. " +
                        "NetworkManager persists across scene loads via DontDestroyOnLoad, so " +
                        "it belongs in the boot scene only.");
                    Destroy(gameObject);
                    return;
                }
                _instance = this;
                _ownsProcessState = true;
            }
            ...
            // One line, once per process, naming what this build IS.
            //
            // 🔑 Unconditional Debug.Log, deliberately, and NOT RtmpeLog.Info:
            // that helper is gated on `enableDebugLogs`, and the integrator whose
            // SDK version we could not establish is precisely the integrator who
            // did not have verbose logs on. A line that only appears once someone
            // knows to switch it on answers the question after it has stopped
            // being asked. It is not WarnGate either — that is a per-tick rate
            // limiter, and there is nothing to rate-limit about a line emitted
            // once in a process's life.
            //
            // ⚠️ RtmpeLog's stated policy is untouched by this. That policy is
            // about which of the SDK's own messages a host app's crash pipeline
            // may ingest, and RtmpeLog.cs states the premise itself: a message
            // emitted at Debug.Log severity is "not ingested by crash
            // pipelines", which is precisely why the gate DOWNGRADES a
            // suppressed error to Debug.Log rather than dropping it. A line that
            // is Debug.Log to begin with is on the safe side of that rule, so
            // this costs a host app's dashboard nothing.
            //
            // ⛔ Below the duplicate-instance branch above, which returns: a
            // duplicate manager announces nothing, or additively loading a scene
            // that carries one prints this a second time and the line stops
            // meaning "this process is running SDK X".
            // ⛔ Composed by a type a shard executes, not written here: this file
            // is compiled by no project in the repository, so a sentence spelled
            // out at this call site can be held only as text.
            Debug.Log(RTMPE.Core.Diagnostics.SdkBanner.Compose(
                RtmpeSdk.Version,
        ...
        private void Start()
        {
            ...
            // active-cap setting, and both cached sender delegates.
            ...
            // Both flush modes send through the batch manager, which is where
            // the "no send exception reaches the Update loop" contract lives —
            // an oversized payload costs that object's update and is counted,
            // never the rest of the tick.  (Counted on every drop; reported at
            // most once a second, carrying what it suppressed.)  Cached once for
            // the same per-tick allocation reason as the delegates above.
            _sendVariableUpdateDelegate = _variableBatchManager.ContainedSingleSender;
        }

        private void Update()
        {
            // Ahead of every other tick: a transport fault whose teardown the
            // dispatcher could not take leaves the session running, and the
            // ticks below would keep driving a connection that no longer has one.
            PumpPendingTransportErrorTeardown();
            ...
            // Drive the JoinRoom retransmit, the CreateRoom expiry and the
            // deadlines on every request the Room Service answers with silence —
            // a property write, a master transfer, a kick — on the same
            // main-thread cadence, so a lost request or reply recovers, or is
            // reported, without the app owning a timer.
            _roomManager?.Tick();

            // Same cadence for the pending JoinLobby deadline.  Its clock was
            // read only while handling a reply, so the case it exists for — a
            // join that draws no reply at all — could not reach it.
            _lobbyManager?.Tick();

            // And for the scene the room is waiting to finish loading. The
            // rendezvous is settled by a server broadcast, so a room that never
            // settles produces nothing at all to be handled — the deadline has
            // to be read from the frame loop or from nowhere.
            _sceneManager?.Tick();
            ...
            // Two cadences on one interval, and two clocks.
            //
            // Each catch-up loop drains its accumulator with a `while` rather
            ...
            // MaxTicksPerFrame caps each loop's catch-up to keep a long pause from
            // turning into a multi-second hitch on resume.  ⚠️ It bounds a LOOP, and
            // there are two: one frame's worst case is twice the ceiling — eight
            // simulation ticks and eight variable flushes.
            //
            ...
            if (flushInterval <= 0f || float.IsNaN(flushInterval)
                || float.IsInfinity(flushInterval))
                flushInterval = DefaultVariableFlushInterval;

            // A non-finite accumulator never satisfies `>= flushInterval`, so
            // neither the loop nor the surplus drop below would ever fire again and
            // the cadence would end silently — the failure InterestManager guards
            // its own accumulator against for the same reason.
            if (!IsUsableAccumulator(_simTickAccum))     _simTickAccum = 0f;
            if (!IsUsableAccumulator(_replicationAccum)) _replicationAccum = 0f;

            // ── The simulation tick, in game time ────────────────────────────
            //
            // ⛔ The one cadence in this method deliberately charged on the
            // game's clock.  It advances _localTick and dispatches OnFixedTick,
            // and OnFixedTick is where NetworkTransform samples and ships
            // client-side-prediction input — so a driver that kept running it at
            // Time.timeScale = 0 would ship the last input it was handed for the
            // whole of a pause menu, and the server would walk a player who had
            // stopped steering.  Its residual is published as
            // SubTickResidualSeconds, which NetworkTransform converts by
            // dividing the scale back out.
            _simTickAccum += Time.deltaTime;
            int simTicksThisFrame = 0;
            while (_simTickAccum >= flushInterval
                   && simTicksThisFrame < MaxTicksPerFrame)
            {
                _simTickAccum -= flushInterval;
                // Only advance while in a room — ticks outside a room are
                // meaningless for CSP.
                ...
                // sub-tick frame rates.
                DispatchFixedTick(flushInterval);
                simTicksThisFrame++;
            }
            if (_simTickAccum >= flushInterval)
            {
            ...
            //
            // A paused client is still a connected one and the room around it
            // keeps playing, so state the game writes while its own clock is
            // stopped still has to reach the other players.  Sharing the
            // simulation's accumulator meant a pause menu silently ended every
            // NetworkVariable's replication for as long as it was open.
            //
            // Runs AFTER the simulation half, so when both fire in one frame
            // a variable an OnFixedTick handler wrote is published in that same
            // frame.  ⚠️ Not every frame: two accumulators on one interval keep
            // independent PHASE, and a pause of an odd number of frames
            // separates them for good — at a frame rate that is a whole
            // multiple of the tick rate, permanently.  What the ordering buys
            // is that the write is never held over a flush that has already
            // happened; the bound on publication is one tick interval, which is
            // what it was before the split as well.
            //
            // ⚠️ And it carries its own counter rather than stamping _localTick.
            // A receiver orders a variable's updates by the tick they arrive
            // with, so a cadence that outlives the simulation tick while still
            // stamping it would emit a run of updates all bearing one number,
            // which the far side reads as a retransmit of a frame it has already
            // applied and drops.
            _replicationAccum += Time.unscaledDeltaTime;
            int flushesThisFrame = 0;
            while (_replicationAccum >= flushInterval
                   && flushesThisFrame < MaxTicksPerFrame)
            {
                _replicationAccum -= flushInterval;
                if (IsInRoom) _replicationTick++;
                FlushDirtyNetworkVariables();
                flushesThisFrame++;
            }
            if (_replicationAccum >= flushInterval)
            {
                // Same surrender as the simulation half.  It matters more here:
                // this accumulator keeps filling while the game's clock is
                // stopped, so without the drop a long pause would bank every
                // second of it and spend the whole backlog eight flushes a frame
                // on resume — publishing nothing new, since a variable is sent
                // once per change, and charging the frame rate for it.
                _replicationAccum = 0f;
            }
            ...
            //
            // On the real clock, like every other deadline this method drives.
            // The deadline itself is wall-clock — RequestIdAllocator compares
            // against MonotonicClock — so charging the sweep that enforces it in
            // game time lets the two disagree by the whole of a pause: the
            // deadline passes and nothing is looking.  A caller awaiting a
            // server RPC is owed an answer or a timeout, and the peer that owes
            // it is not slowed by this game's Time.timeScale.
            _rpcPurgeAccum += Time.unscaledDeltaTime;
            ...
            // Real time, for the reason the sweep above gives: the TTL these
            // entries are aged against is a Stopwatch reading, so a cadence
            // charged in game time cannot keep the promise that an entry is
            // never more than one period late on its expiry.
            _pendingDespawnPruneAccum += Time.unscaledDeltaTime;
            ...
            // Resume the staged catch-up release.  Bounded per frame so a
            // 1000-object room does not land in one, and finished within about a
            // second so nothing downstream is asked to hold across it —
            // self-gates on an empty buffer and costs one Count read otherwise.
            if (IsInRoom)
        ...
        private void FlushDirtyNetworkVariables()
        {
            ...
            for (int i = 0; i < _flushScratch.Count; i++)
            {
                ...
                if (nb == null) continue;
                // A NetworkVariable belongs to the component that declared it,
                // and the registry entry is only where the object is reached —
                // so the flush is offered to every component of the object.
                RTMPE.Core.ObjectDispatchOps.FlushAll(
                    nb.ObjectComponents, sender, _flushFaultReporter);
            }
            _flushScratch.Clear();
        ...
        /// </summary>
        // A tick accumulator is usable while it is finite.  NaN poisons every
        // subsequent `+=` and compares false against everything, so a single one
        // would end the cadence it drives with no error and no diagnostic — the
        // reason InterestManager clamps its own interval before consuming it.
        private static bool IsUsableAccumulator(float accumulator)
            => !float.IsNaN(accumulator) && !float.IsInfinity(accumulator);
        ...
        private void DispatchFixedTick(float dt)
        {
            ...
            for (int i = 0; i < _fixedTickScratch.Count; i++)
            {
                ...
                if (nb == null) continue;
                // The registry holds one component per object; the tick belongs
                // to every component that implements it.  Client-side prediction
                // lives on NetworkTransform, which sits wherever the prefab
                // author put it.
                RTMPE.Core.ObjectDispatchOps.FixedTickAll(
                    nb.ObjectComponents, dt, _fixedTickFaultReporter);
            }
            _fixedTickScratch.Clear();
        }

        // Cached so the per-frame fan-out allocates no closure.  Rate-limited
        // because a component that throws on flush does so on every tick.
        // ⚠️ Its sibling below is deliberately NOT rate-limited: a FixedTick
        // fault is per-object-per-tick where this is per-component-per-frame,
        // and that reporter predates this one.
        private static long _lastFlushFaultWarnTicks;

        private static readonly Action<RTMPE.Core.INbDispatch, Exception> _flushFaultReporter =
            (component, ex) =>
            {
                if (!RTMPE.Core.WarnGate.ShouldEmit(ref _lastFlushFaultWarnTicks)) return;
                Debug.LogError(
                    $"[RTMPE] Variable flush threw {ex.GetType().Name} on " +
                    $"{component?.GetType().Name ?? "<null>"}: {ex.Message}. This tick's " +
                    "update for that component was lost — the variables it had already " +
                    "appended were marked clean and the payload was discarded — so they have " +
                    "been re-marked dirty and will be re-sent whole on the next tick. The " +
                    "components after it in the same object are no longer taken down with it.");
            };

        // Cached so the per-tick fan-out allocates no closure.  A single
        // misbehaving subclass must not abort the rest of the dispatch.
        private static readonly Action<RTMPE.Core.INbDispatch, Exception> _fixedTickFaultReporter =
            (component, ex) =>
            {
                Debug.LogError(
        ...
        private void OnDestroy()
        {
            ...
            // ⛔ A component that never won the singleton race owns none of the
            // state below, and every line of it is process-wide. Returning here
            // is the whole of LIFECYCLE-02: a duplicate destroyed at Awake used
            // to null the LIVE manager's verifier hooks and logger settings on
            // its way out.
            //
            // Nothing is skipped that the duplicate owns: Awake returns before
            // it subscribes to either SceneManager event, before it starts a
            // coroutine and before it builds a transport, so there is nothing
            // of its own to unsubscribe, stop or dispose.
            if (!_ownsProcessState) return;
