...
namespace RTMPE.Core
{
    ...
    public sealed class NetworkObjectRegistry
    {
        ...
        /// in the dependency stack.  The overload taking an <c>out</c>
        /// parameter names the evicted instance, which is what makes that
        /// obligation dischargeable rather than merely stated; a caller that
        /// discards it keeps the slot in its own ledger and leaves the evicted
        /// instance's teardown unclaimed, and that teardown reconciles by
        /// object id — an id this call has just given to somebody else.
        /// </para>
        /// <para>
        ...
        /// </summary>
        /// <returns>
        /// <see langword="true"/> when the object now occupies its slot.
        /// <see langword="false"/> when the registration was refused — the
        /// caller is holding a live object nothing will route to, and must
        /// not report a spawn it did not get.
        /// </returns>
        public bool Register(NetworkBehaviour obj) => Register(obj, out _);

        /// <summary>
        /// Register <paramref name="obj"/>, reporting in
        /// <paramref name="evicted"/> the instance that held the slot before
        /// it — <see langword="null"/> when the slot was free, when the object
        /// was already in it, or when the registration was refused.
        /// </summary>
        /// <remarks>
        /// The live-spawn count sits above this type in the dependency stack,
        /// so an eviction travels back to the caller for the count the evicted
        /// instance was charged to be returned.  Returning it is also what lets
        /// the caller claim that instance's teardown: the reconciliation such a
        /// claim suppresses is keyed by object id, and this call has just given
        /// the id to somebody else, so left unclaimed it unregisters a live
        /// object rather than the one being destroyed.
        /// </remarks>
