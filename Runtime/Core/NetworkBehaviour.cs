    ...
    public abstract class NetworkBehaviour : MonoBehaviour, INbLifecycle
    {
       ...
       /// <para>Must be called from the Unity main thread while connected and in a room.</para>
        ...
        internal void SetOwner(string newOwner)
        {
            ...
            // The interpolator's sender-tick high-water is the same kind of gate
            // over the same kind of counter, and it was not covered by the loop
            // above.  When the render timeline is driven by the owner's input
            // tick, that tick belongs to whichever client owns the object; after
            // a handover the new owner's counter can sit far below the previous
            // owner's, and every inbound record is rejected as a stale replay
            // until it catches up — a replica frozen for minutes while its
            // traffic keeps arriving.  Retire the estimator here, on the one
            // event that changes whose clock the ticks are drawn from.
            CachedNetworkTransformInterpolator?.ResetSenderTimeline();
