...
namespace RTMPE.Core
{
    ...
    public sealed class NetworkSettings : ScriptableObject
    {
        ...
        [Tooltip("This client's simulation cadence in Hz: how often the local tick cursor " +
                 "advances, network variables flush, and a changed transform may be broadcast.\n\n" +
                 "It does NOT set the server's rate. Rooms broadcast at a fixed 30 Hz, and the " +
                 "receive path converts server ticks with that rate regardless of this value.\n\n" +
                 "30 matches the server, so every sample the client produces is one the server " +
                 "consumes. Above 30 the tick engine keeps only the latest sample per tick and the " +
                 "surplus counts against the per-session state budget — past that budget the server " +
                 "sheds frames, which surfaces as jitter. Below 30 the client under-samples its own " +
                 "motion, and peers see it at the lower rate.")]
        [Range(1, 128)]
        ...
        [Tooltip(
            ...
            "journal during testing, together with the errors the SDK reports " +
            "about itself — which reach this channel at their real severity even " +
            "when Enable Debug Logs is off and the console is showing them as " +
            "ordinary lines. The SDK's own warnings follow Capture Warnings " +
            "below, like every other warning. Leave OFF in production: it " +
            "captures the whole process's logs, which may include anything the " +
            "game code prints.")]
        public bool enableDiagnosticsUplink = false;

        [Tooltip(
            "Also capture Warning logs (in addition to Error/Exception/Assert), " +
            "including the warnings the SDK reports about itself. Off by default " +
            "to keep the uplink focused on faults.")]
        ...
        [Tooltip("UDP socket SO_RCVBUF size in bytes. Default 256 KiB sized " +
                 "for 30 Hz 16-player sessions (~480 datagrams/second). " +
                 "Smaller values cause silent kernel-side datagram drops, and " +
                 "below one maximal datagram the largest server replies are " +
                 "dropped by the kernel every time.")]
        [Range(RTMPE.Protocol.PacketBuilder.MaxDatagramBytes, 4_194_304)]
        public int receiveBufferBytes = 262_144;

        [Tooltip("Size of the scratch buffer the network thread reads each " +
                 "datagram into. A datagram larger than this is lost with no " +
                 "diagnostic — truncated by the socket and failing its " +
                 "authentication tag on POSIX, refused with WSAEMSGSIZE on " +
                 "Windows — so the minimum is the largest datagram a UDP path " +
                 "carries, not a smaller number chosen for the traffic you " +
                 "expect.")]
        ...
        [Tooltip("Maximum number of room entries the lobby parser will accept " +
                 ...
                 "allocation from a malicious or buggy server. A reply larger " +
                 "than this is refused whole, so the minimum is the 100 rooms " +
                 "the Room Service may itself return, not a smaller number of " +
                 "your choosing.")]
        ...
        [Tooltip("How long a networked scene load may go unsettled before " +
                 "NetworkSceneManager.OnSceneLoadTimedOut is raised, in seconds. " +
                 "The room reaches all-loaded only once every seat has reported, so " +
                 "one client that never does leaves the rest waiting; this bounds the " +
                 "wait with a report rather than shortening it. Leave at 0 for the " +
                 "SDK default; set -1 to switch the report off.")]
        ...
        [Tooltip("The generated prefab registry. Assign it and every prefab in " +
                 "the Editor ledger is registered for spawning automatically; " +
                 "leave it empty to register by hand with SpawnManager.RegisterPrefab. " +
                 "Write it from Window → RTMPE → Network Prefabs.")]
        ...
        [Tooltip("Hard cap on the apparent owner velocity (world units per " +
                 ...
                 "which carries the cap's baseline to the destination so the jump " +
                 "is not measured as travel — the cap still governs movement " +
                 "after it. Set 0 to disable (back-compat).")]
        [Range(0f, 1_000f)]
        ...
        [Tooltip("Seconds between periodic NetworkVariableList full-sync " +
                 "snapshots.  A list ships steady-state edits as a delta against " +
                 "the contents the receiver is assumed to hold, and delivery is " +
                 "best-effort whenever the ARQ extension is not negotiated — so " +
                 "without this refresh one lost datagram leaves owner and " +
                 "replica permanently apart, with nothing that would notice.  " +
                 "Costs one snapshot per list per interval.  0 disables it.")]
        [Range(0f, 300f)]
        public float networkVariableListFullSyncIntervalSeconds =
            DefaultNetworkVariableListFullSyncIntervalSeconds;

        /// <summary>
        /// The cadence a project gets without touching the setting, and what a
        /// value from outside the Inspector's floor is restored to.  Declared
        /// once so the initialiser and the clamp below cannot drift apart.
        /// </summary>
        ...
        [Header("Crypto")]
        [Tooltip(
            "64-character lowercase hex string of the 32-byte Ed25519 static public key of the " +
            "gateway you expect to connect to (H4 server pinning).\n\n" +
            "Copy from the gateway startup log or developer dashboard.\n\n" +
            "Required under the default Server Pinning Mode of Strict, which refuses to connect " +
            "without it. Only TrustOnFirstUse and InsecureNoPinning proceed while this is blank.")]
        public string pinnedServerPublicKeyHex = "";

        [Tooltip(
            "REQUIRED. 64-character lowercase hex string of the gateway's static X25519 public " +
            "key.\n\n" +
            "The SDK seals the API key to this key in HandshakeInit and the gateway opens it with " +
            "the matching private key — so your build carries no shared secret.  It is the only " +
            "API-key envelope the gateway accepts; leaving this blank means no connection.\n\n" +
            "Obtain it from the developer dashboard (the Sealed-Box Public Key) or the " +
            "gateway-config endpoint.  This is an X25519 key, distinct from the Ed25519 Pinned " +
        ...
        /// public key, or return <see langword="null"/> when it is unset — which
        /// leaves the SDK with no API-key envelope and therefore no handshake.
