...
#warning RTMPE does not run in a WebGL player: its I/O layer needs a background thread the browser does not provide, and a transport installed through NetworkManager.SetTransportFactory sits above that thread rather than replacing it.
        ...
        /// <para>
        /// A no-op, with an error logged and no state change, in a WebGL player:
        /// the SDK drives its transport from a background thread that platform
        /// does not provide. Supported platforms are Windows, macOS, Linux,
        /// Android and iOS.
        /// </para>
        /// </summary>
            ...
            lock (_sceneTransitionLock)
            {
                ...
                _matchmakingManager = new MatchmakingManager(
                    ...
                    entry => _roomManager.EnterMatchmadeRoom(
                        ...
                        entry.Players, entry.PropertiesPayload, entry.PropertiesComplete));
        ...
        /// token.  Skips the envelope open and the PostgreSQL API-key lookup;
        ...
        /// <see langword="false"/> when no reconnect token is held, the manager
        /// is already connected / reconnecting, or the player is one the SDK
        /// does not support — see <see cref="Connect(string)"/>.
        /// </returns>
        ...
        /// <para>A no-op when already disconnected or disconnecting — <b>except</b>
        /// while a bounded reconnect is in flight. Between attempts that loop
        /// waits in <see cref="NetworkState.Disconnected"/>, so treating that
        /// state as idle would make this call do nothing and let the loop rebuild
        /// the session a moment later. Called there, it retires the loop and
        /// clears the session, and the application observes
        /// <c>Disconnected → Disconnecting → Disconnected</c> with a second
        /// <see cref="OnDisconnected"/> carrying
        /// <see cref="DisconnectReason.ClientRequest"/> — the first having been
        /// raised by the attempt timeout that put it in
        /// <see cref="NetworkState.Disconnected"/>. A handler that must act once
        /// per session should key on the reason, or on
        /// <see cref="NetworkState.Connected"/> having been reached.</para>
        /// <para>A no-op after <c>Cleanup()</c>, always.</para>
        /// </summary>
        ...
        /// <summary>
        /// Enqueue a raw packet for transmission. <b>Main thread only</b> — see
        ...
        ///
       /// <para><b>Threading:</b> this reads negotiated session state without
        /// synchronisation, so it must be called from the Unity main thread.
        /// The defensive copy above makes the caller's <em>buffer</em> safe to
        /// reuse; it does not make the call safe to make from a worker. To send
        /// from another thread, marshal through
