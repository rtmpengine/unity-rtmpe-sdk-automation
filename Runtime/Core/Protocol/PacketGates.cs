...
namespace RTMPE.Core.Protocol
{
    ...
    internal static class PacketGates
    {
        ...
        /// <summary>
        /// Strict allow-list of every opcode the SDK may receive.
        /// <para>
        /// This gate runs on <b>inbound</b> headers only, so it is the
        /// server→client and bidirectional half of <see cref="PacketType"/> —
        /// not the whole enum.  Client→server-only opcodes are deliberately
        /// absent: <see cref="PacketType.Diagnostics"/> (0x0C) and
        /// <see cref="PacketType.PhysicsSync"/> (0x45) are sent by this SDK and
        /// never sent to it, and admitting them here would grant a direction
        /// the protocol does not define, which is the same reasoning that keeps
        /// <see cref="PacketFlags.SealedApiKey"/> out of
        /// <c>PacketProtocol.KNOWN_FLAGS</c>.
        /// </para>
        /// <para>
        /// A new opcode the SDK can <i>receive</i> must be added here in the
        /// same commit as the enum and <c>PacketType::try_from</c> in
        /// <c>modules/gateway/src/packet/header.rs</c>, or the receiver treats
        /// the new frame as <c>UnknownType</c>.  A new client→server opcode
        /// must not be.
        /// </para>
        /// </summary>
        ...
        internal static string DescribeHandshakeError(byte code)
        {
            switch (code)
            {
                ...
                case HandshakeErrorInvalidFormat:
                    return "the handshake was malformed — verify the gateway's sealed-box public key and the pinned server key";
