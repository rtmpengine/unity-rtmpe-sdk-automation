...
namespace RTMPE.Core
{
    public sealed partial class NetworkManager
    {
        ...
        // ── Legacy-opcode rejection test seam ─────────────────────────────────

        /// <summary>
        /// Drives <see cref="OnHandshakeAck"/> from a test after forcing the
        /// state machine into <see cref="NetworkState.Connecting"/> via
        /// <see cref="ForceConnectingStateForTests"/>, so a test can observe
        /// that the legacy opcode changes nothing.
        ...
        /// <summary>
        /// Re-arms the once-per-second subscriber-throw report, so a case that
        /// expects it does not inherit the previous case's spend. Do NOT call
        /// from production code.
        /// </summary>
        internal static void ResetSubscriberThrowWarningForTests()
            => System.Threading.Interlocked.Exchange(ref _lastSubscriberThrowWarnTicks, 0L);
       ...
       // Invariant: NEVER call `handler.Invoke(...)` directly on a public
        ...
        // A subscriber that throws throws every time its event is raised, and
        // `SafeRaise` carries the 30 Hz state path as well as the room, spawn and
        // RPC events — so this is two console writes and a stack capture per
        // datagram until the subscriber is fixed.
        //
        // ⚠️ One budget for every event this class raises. The line names the
        // event, so a handler failing on a busy one can mask a rarer one for a
        // second at a time; that is the price of not keeping a gate per event
        // name, and the alternative — a table keyed by a string the caller
        // supplies — is a per-packet lookup on the path this exists to make
        // cheap.
        ...
        {
            if (!RTMPE.Core.WarnGate.ShouldEmit(ref _lastSubscriberThrowWarnTicks)) return;
