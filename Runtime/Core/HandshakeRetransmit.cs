...
//     leading AEAD nonce.  A re-emission of an Init that was lost in flight is
//     seen as fresh and draws the Challenge; one whose Round 1 has already
//     replied is answered from that slot with the same Challenge again, which is
//     what makes a lost *answer* recoverable rather than only a lost request;
//     one arriving while that Round 1 is still in its database lookup finds
//     nothing yet and is refused, which is why the ladder has four rungs and not
//     one; and one whose
//     Round 2 has already completed finds no slot and is refused as a replay,
//     changing no server state.  The bytes are unchanged in every case, so the
//     transcript the client later verifies against is unchanged.
