...
namespace RTMPE.Core
{
    ...
    public static class PacketProtocol
    {
        ...
        /// <summary>
        /// Current protocol version byte. Must match gateway constant <c>VERSION = 5</c>.
        /// A mismatch causes the gateway to reject the packet with a version error.
        /// <para>
        /// Moved from 4 when a <c>NetworkVariable</c>'s wire identity became
        /// derived and <c>var_id</c> widened from two bytes to four. A
        /// <see cref="PacketType.VariableUpdate"/> entry is
        /// <c>[var_id:4][value_len:2][value_bytes]</c>, so a build still framing
        /// at the old width reads the high half of the identity as the length and
        /// mis-frames every entry after the first.
        /// </para>
        /// <para>
        /// Moved from 3 when rigidbody state left <see cref="PacketType.StateSync"/>
        /// for <see cref="PacketType.PhysicsSync"/>. The two grammars that shared
        /// 0x40 had colliding lengths, so a build that still emits physics under
        /// 0x40 has its frames read at a transform's offsets — silently, and with
        /// no signal back to the sender.
        /// </para>
        /// <para>
        /// The reason is the same both times: refusing the header is where the
    ...
    public enum PacketType : byte
    {
        ...
        //  its byte budget and kept the version alone.
        //
    ...
    /// </para>
    /// <para>
    /// ⚠️ That upgrade has not happened.  The gateway's negotiation ceiling is
    /// <see cref="WireFormatVersion.V2"/>, because the gate that would verify
    /// a V4 buffer is reached from nowhere on its ingest path; a V4 preference
    /// is therefore clamped, and the outcome is not returned to the SDK.  This
    /// constant selects what the SDK <em>advertises</em> — advertising above
    /// the server's ceiling is the correct use of a min-negotiation and costs
    /// nothing — but flipping it is not what turns V4 on, and nothing in this
    /// SDK emits the typed table today.
    /// </para>
    /// <para>
    ...
    /// <summary>
    /// Who may address a spawned object with an Enhanced RPC (audit
    /// <c>S4-02</c>).
    ///
    /// <para>Declared by the spawner in one optional trailing byte on the
    /// <c>Spawn</c> (0x30) payload, after the fixed 28-byte transform tail:
    /// the pre-<c>S4-02</c> payload is exactly <c>42 + owner_len</c>, so
    /// <c>43 + owner_len</c> carries a declaration and anything shorter does
    /// not.</para>
    ///
    /// <para>MUST stay in sync with <c>SPAWN_AUTHORITY_SHARED_BIT</c> and
    /// <c>classify_spawn_declared_authority</c> in
    /// <c>modules/gateway/src/nats/forwarder.rs</c>.</para>
    ///
    /// <para>⚠️ The packet-constant sync guard does NOT cover this pair: it
    /// compares <c>packet/header.rs</c> against this file, and this bit is
    /// declared in the forwarder instead. What actually holds the two sides
    /// together is a byte vector each asserts independently —
    /// <c>SPAWN_WIRE_VECTOR_SHARED</c> there, <c>SpawnAuthorityWireVectorTests</c>
    /// here — so a drift shows up as a failed vector rather than as a passing
    /// guard that was never looking.</para>
    /// </summary>
    public static class SpawnAuthorityFlags
    {
        /// <summary>
        /// Bit 0 — any member of the room may address this object.  Set only by
        /// an explicit declaration.
        /// </summary>
        public const byte Shared = 0x01;

        /// <summary>
        /// The value emitted for an owner-only object, and the meaning the
        /// gateway assigns to an absent declaration and to every unrecognised
        /// bit pattern.
        ///
        /// <para>The asymmetry is the point: this field travels in a payload a
        /// modified client controls, so it can only be allowed to *give up*
        /// authority, never to claim it.  Truncating the byte away, or setting a
        /// reserved bit a future release has not defined yet, both resolve to
        /// owner-only.</para>
        /// </summary>
    ...
    /// <summary>
    /// The object-lifetime declaration a <c>Spawn</c> payload carries, in the
    /// byte after <see cref="SpawnAuthorityFlags"/>.  Mirrors
    /// <c>SPAWN_DESTROY_WITH_OWNER_BIT</c> on the server, and the two are held
    /// to one another by a known-answer vector of the whole payload that both
    /// sides assert byte for byte — the SDK's copy lives in
    /// <c>SpawnAuthorityWireVectorTests</c>.  A value changed on one side alone
    /// fails there rather than on the wire.
    /// </summary>
    /// <remarks>
    /// The gateway keeps one ownership record per live object and bounds a room
    /// by how many it holds.  When a player leaves, every client tears down that
    /// player's objects whose component says to and hands the rest to the room's
    /// host — a decision taken entirely on each client, announced to nobody. The
    /// gateway therefore cannot tell a torn-down object from a surviving one and
    /// has to keep every record, so a room that never empties spends a slot of
    /// its ceiling on each departure. This byte is what tells it apart.
    /// <para>The same asymmetry as the authority byte governs it: an absent or
    /// unrecognised value means the record is kept, so truncating the field away
    /// can only make the gateway more careful with an object, never less.</para>
    /// </remarks>
    public static class SpawnLifetimeFlags
    {
        /// <summary>
        /// Bit 0 — every client destroys this object when its owner leaves the
        /// room, so no receiver is left for its ownership record to protect.
        /// </summary>
        public const byte DestroyWithOwner = 0x01;

        /// <summary>
        /// The value emitted for an object that outlives its owner's departure,
        /// and the meaning of an absent or unrecognised declaration.
        /// </summary>
