...
- A `NetworkSettings` asset (optional but recommended). Create one via
  **Assets → Create → RTMPE → Settings** and fill in
  `apiKeySealServerPublicKeyHex` with your gateway's X25519 public key.
...
6. (Optional, but recommended.) Open **Edit → Project Settings → RTMPE**
   ...
   and `Api Key Seal Server Public Key Hex` with your gateway's X25519 key.
